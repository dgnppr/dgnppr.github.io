; (function () {
    ...
    function link(content) {
        ...
        // (태그, 타이틀 처리) "[[#tagName#]]{text}"를 <a href="/concept/tagName#">text</a> 로 replace하여 링크를 만든다.
        //                           [[#           #]]      {            }
        content = content.replace(/\[\[#([^\[\]]+?)#\s*\]\]\{([^\{\}]+?)\}/g,
            '<a href="/tags#$1" class="inner-link labeled-link" data-name="$1"><sup class="tagged-link"/></sup>$2</a>');

        // (태그 처리) "[[#tagName#]]"을 <a href="/concept/tagName#">tagName</a> 로 replace하여 링크를 만든다.
        //                           [[#           #]]
        content = content.replace(/\[\[#([^\[\]]+?)#\s*\]\]/g,
            '<a href="/tags#$1" class="inner-link labeled-link" data-name="$1"><sup class="tagged-link"/></sup>$1</a>');

        // (추가 타이틀 처리) 다음과 같은 문자열을 <a href="/concept/document">document-name</a> 으로 replace하여 링크를 만든다.
        //  [[document]]{document-name}       => <a href="/concept/document">document-name</a>
        //  [[/document]]{document-name}      => <a href="/concept/document">document-name</a>
        //  [[/dir/document]]{document-name}  => <a href="/concept/dir/document">document-name</a>
        content = content.replace(/\[\[\/?([^\[\]]+?)\s*\]\]\{([^\{\}]+?)\}/g,
            '<a href="/concept/$1" class="inner-link labeled-link" data-name="$1">$2</a>');

        // "[[document]]"가 있다면 <a href="/concept/document">document</a> 와 같이 replace하여 링크를 만든다.
        //  예제는 (추가 타이틀 처리)와 거의 비슷하다.
        content = content.replace(/\[\[\/?(.+?)\s*\]\]/g,
            '<a href="/concept/$1" class="inner-link no-labeled-link" data-name="$1">$1</a>');
