...
**목표:** 그래프 기반 기획 → 사실 검증 → 기술 심화 → 직설적 기술 집필 → 발행+그래프 연결까지 한 흐름으로, 데이터 엔지니어링·클라우드 글을 **연결된 지식 자산**으로 생산한다. 온톨로지를 워크플로우에 Full-flywheel로 결합한다(`ontology_gaps`로 주제 → 발행 시 `relations` 자동 연결 → `make ontology` → 다음 글 제안).
