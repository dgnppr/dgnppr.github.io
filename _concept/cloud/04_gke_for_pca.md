...
> "Autopilot을 썼는데 왜 GPU 워크로드가 안 되죠?" — Autopilot의 관리 편의성에 이끌려 선택했다가 제약에 막히는 가장 흔한 패턴이다. 이 글은 Standard와 Autopilot 선택이 GKE 설계의 시작점임을 보이고, **PCA 시험의 GKE 문제가 결국 "어떤 모드·구성이 요구사항에 맞는가"라는 의사결정**임을 보인다. 이 글은 PCA 준비 시리즈 3편으로, VPC(1편) 위에 컨테이너 오케스트레이션을 쌓는다.
...
> "Control Plane 업그레이드 중에도 API가 가용해야 한다" → **Regional 클러스터**. Control Plane이 3존에 분산되어 있어 단일 존 장애·업그레이드에도 API Server가 살아있다.
