...
시험 지문은 "몇 R" 같은 용어를 직접 쓰지 않는다. 대신 아래 신호어로 전략을 유도한다. 지문의 동사·형용사에서 변경 의지와 시간 제약을 읽어 R을 역산하라.

| 지문 신호어 | 유도되는 전략 | GCP 경로 |
|------------|--------------|----------|
| "가장 빠르게", "최소한의 변경으로", "코드 수정 없이", "데이터센터 폐쇄 기한이 촉박" | **Rehost** | Migrate to VMs(+테스트 클론) |
| "운영부담을 줄이면서", "패치·백업을 관리형에 위임", "DB를 매니지드로" | **Replatform** | 자가관리 DB → Cloud SQL/AlloyDB, VM 컨테이너 → GKE/Cloud Run |
| "클라우드 네이티브로 재설계", "확장성·탄력성을 최대화", "모놀리스를 분해" | **Refactor / Re-architect** | GKE 마이크로서비스 + Spanner/Pub/Sub/BigQuery |
| "상용 SaaS로 대체", "자체 운영 중단하고 구독으로" | **Repurchase** | SaaS 전환(예: 자체 메일/CRM → 구독) |
| "규제·레거시 종속으로 당분간 온프렘 유지" | **Retain** | 하이브리드 연결(Interconnect/VPN)로 연동 |
| "더 이상 쓰지 않는", "사용률 0에 가까운" | **Retire** | 인벤토리에서 제외 |

<div class="callout-warning">
함정: <strong>"관리형으로 운영부담을 줄인다"</strong>는 신호는 Replatform이지 Rehost가 아니다. Rehost(Migrate to VMs)는 VM을 그대로 옮길 뿐 DB를 Cloud SQL로 바꾸지 않는다. 반대로 <strong>"6주 안에 최소 변경으로 데이터센터를 비운다"</strong>인데 Refactor(재설계)를 고르면 기한·리스크가 폭증해 오답이다. 변경량이 클수록 시간·비용·리스크가 함께 커진다는 정비례 관계를 지문에 대입하라.
</div>
...
- **Backup and DR Service**: 관리형 백업·복구 서비스로 VM(Compute Engine)·DB 등의 백업을 중앙에서 정책 기반으로 관리한다. 개별 스냅샷을 수동으로 돌리는 대신 백업 계획(주기·보존 기간)·복구 오케스트레이션·중앙 관리 콘솔을 제공하는 것이 차별점이다. 애플리케이션 정합성(application-consistent) 백업과 장기 보존을 한 서비스에서 다룬다. (지원 워크로드·정확한 보존 한도는 시기에 따라 달라지므로 공식 문서로 재확인.)
- **DB 자동 백업 + PITR**: Cloud SQL 등은 자동 백업과 특정 시점 복구(Point-in-Time Recovery)를 제공해 RPO를 줄인다.

<div class="callout-note">
스냅샷·라이프사이클을 손으로 짜는 것과 <strong>Backup and DR Service</strong>의 갈림길: "디스크 하나를 가끔 백업" 수준이면 PD 증분 스냅샷으로 충분하다. 반면 <strong>"다수 VM·DB의 백업을 정책으로 표준화하고, 보존·복구를 중앙에서 관리·자동화"</strong>가 신호어면 Backup and DR Service다. 시험에서 "여러 워크로드의 백업 정책을 일관되게 관리하고 복구를 오케스트레이션"이 나오면 개별 스냅샷 스크립트가 아니라 관리형 서비스를 고른다.
</div>

<div class="callout-warning">
백업 함정: <strong>백업은 복원 테스트를 해야 백업이다.</strong> 그리고 백업을 원본과 같은 리전에만 두면 리전 장애 시 함께 잃을 수 있다 — DR용 백업은 <strong>다른 리전(또는 멀티리전 버킷)</strong>에 두는 것이 원칙이다. "리전 DR인데 백업이 같은 리전"은 설계 결함. 또 백업(과거 시점 복원)과 복제(실시간 이중화)를 혼동하지 말 것 — 백업은 RPO를 백업 주기만큼 남기고, RPO 0은 복제의 영역이다.
</div>
...
### 실행 도구 한눈에 무엇을 옮기나 × 결정 기준

같은 "마이그레이션"이라도 옮기는 대상(VM·DB·객체·분석데이터)에 따라 도구가 갈린다. 대상 축으로 먼저 도구를 좁힌 뒤, 다운타임·대역폭·용량·기한으로 세부 방식을 정한다.

| 옮기는 대상 | 도구 | 결정 축 | 신호어 |
|------------|------|---------|--------|
| **VM(OS·앱 통째)** | **Migrate to Virtual Machines** | 다운타임(스트리밍 컷오버) · 소스(VMware=에이전트리스) | "코드 변경 없이 VM 그대로", "최소 다운타임 리호스트" |
| **운영 DB** | **Database Migration Service** | 다운타임(CDC) · 엔진(동질/이질) | "운영 중 DB를 최소 다운타임으로", "Oracle→Postgres 변환" |
| **객체·파일(온라인)** | **Storage Transfer Service** | 대역폭 · 반복성 · 소스(타 클라우드/온프렘) | "대량·반복 전송", "S3/Azure→GCS 스케줄" |
| **객체·파일(오프라인)** | **Transfer Appliance** | 용량 ÷ 대역폭 = 시간 > 기한 | "저대역폭 + 대용량 + 마감", "온라인이면 수개월" |
| **분석 데이터(→BQ)** | **BigQuery Data Transfer Service** | 정기성 · 소스(SaaS/DW) | "Redshift/Teradata/SaaS를 BigQuery로 정기 적재" |

<div class="callout-warning">
가장 잦은 헷갈림: <strong>DB를 "파일"로 착각해 STS/Transfer Appliance로 옮기는 오답</strong>. 운영 DB의 일관된 이전은 데이터 디렉터리 복사가 아니라 <strong>DMS</strong>의 일이다. 그리고 <strong>STS vs Transfer Appliance</strong>는 온라인/오프라인 이분법이 아니라 "용량 ÷ 실효 대역폭 = 시간"이 사업 기한 안에 드는가로 갈린다 — 회선이 충분하면 대용량도 STS, 회선이 부족하면 소·중용량도 오프라인이 나을 수 있다.
</div>
...
## 공식 케이스 스터디와 마이그레이션·DR 접점

PCA 시험은 4개 공식 케이스 스터디(EHR Healthcare · Helicopter Racing League · Mountkirk Games · TerramEarth)를 기반으로 케이스당 여러 문항을 낸다. 이 도메인에서는 그중 **EHR Healthcare(온프렘→GCP 마이그레이션이 서사의 중심)**와 **TerramEarth(데이터 파이프라인 이전·현대화)**가 가장 자주 맞닿는다. 케이스별 세부 수치는 시험장의 v6.1 원문으로 확인하고, 아래는 마이그레이션·DR 관점에서 요구사항을 설계 결정으로 잇는 매핑이다.

### EHR Healthcare 온프렘 레거시의 컨테이너 마이그레이션 + 멀티리전 가용성

EHR Healthcare는 멀티병원 대상 EHR SaaS로, 요구의 핵심이 이 도메인과 정면으로 겹친다 — 온프렘 레거시를 컨테이너화해 GCP로 이전하고, 이전 기간에도 온프렘과 상시 연동하며, 멀티리전 고가용성(99.99% 목표)과 무손실을 달성하는 것.

| 케이스 요구사항 | 마이그레이션·DR 설계 결정 |
|----------------|---------------------------|
| 레거시 애플리케이션을 컨테이너로 현대화하며 이전 | 단순 Rehost(Migrate to VMs)가 아니라 **Replatform → GKE**. "컨테이너화하며 옮긴다"는 신호가 핵심 |
| 이전 기간 중 온프렘 레거시와 지속 연동 | **Dedicated Interconnect**(안정적 저지연 상시 연동). 일회성 전송 도구와 구분 |
| 여러 병원 데이터의 관리형 DB 이전 | **Database Migration Service** 연속 복제로 최소 다운타임 컷오버 |
| 멀티리전 99.99% 가용성, 데이터 손실 최소화 | 리전 장애까지 견디는 **멀티리전/교차 리전 복제** DR 설계, 관리형 DB의 교차 리전 구성 |
| 전 구간 저장·전송 암호화, 접근 감사 | 백업·복제 데이터에도 CMEK 적용(상세는 [[/concept/cloud/09_security_for_pca]]) |

<div class="callout-warning">
EHR 함정: "온프렘을 GCP로 옮긴다"만 보고 <strong>Migrate to VMs(Rehost)</strong>를 반사적으로 고르면 안 된다. 케이스가 <strong>"컨테이너화하며 현대화"</strong>를 명시하면 답은 <strong>Replatform(GKE)</strong> 쪽이다. 또 "이전 중 온프렘 연동"을 일회성 전송 도구(Transfer Appliance)로 풀면 어긋난다 — 상시 연동은 <strong>Interconnect/VPN</strong>의 일이다.
</div>

### TerramEarth 텔레메트리 데이터 파이프라인의 이전과 현대화

TerramEarth는 대규모 중장비 차량군의 텔레메트리를 수집·분석하는 케이스로, "기존 배치 기반 데이터 처리를 확장 가능한 파이프라인으로 이전"이 이 도메인의 접점이다. 차량 대수·성장률 같은 수치는 케이스 원문 기준으로 확인하되, 설계 결정의 방향은 아래로 고정된다.

| 케이스 요구사항 | 마이그레이션·DR / 데이터 이전 결정 |
|----------------|-----------------------------------|
| 기존 대량 배치 업로드(파일) 이전 | **Storage Transfer Service**(온라인 대량) 또는 초기 대량이 저대역폭이면 **Transfer Appliance** |
| 배치 → 실시간 텔레메트리 수집으로 현대화 | **Pub/Sub 수집 + Dataflow 처리**로 파이프라인 재설계(Replatform/Refactor) |
| 차량ID+타임스탬프 시계열 저장 | **Bigtable**(저지연 대량 시계열 쓰기) |
| 딜러/분석용 데이터 웨어하우스로 통합 | **BigQuery**, 외부 DW·SaaS 소스는 **BigQuery Data Transfer Service**로 정기 적재 |
| 예지정비 ML | Vertex AI(현 Gemini Enterprise Agent Platform으로 전환 중) — 상세는 [[/concept/cloud/12_genai_for_pca]] |

<div class="callout-note">
TerramEarth에서 마이그레이션·DR 도메인이 담당하는 조각은 <strong>"기존 데이터를 어떻게 옮기고(STS/Appliance/BQ DTS), 파이프라인을 어떻게 이전·현대화하나(배치→Pub/Sub+Dataflow)"</strong>다. 실시간 스트리밍 아키텍처 자체의 상세는 데이터 처리 도메인이지만, "기존 배치 시스템을 어떤 도구로 옮기나"는 이 글의 전송 도구 선택(온라인 vs 오프라인 vs BQ DTS)과 직결된다.
</div>

### HRL·Mountkirk의 접점(가용성·글로벌 배포)

나머지 두 케이스는 이 도메인과의 접점이 상대적으로 얕지만 가용성·글로벌 배포 관점에서 나올 수 있다. **Helicopter Racing League**는 글로벌 저지연 스트리밍·관측성 강화 요구가 있어 멀티리전 배포·글로벌 LB(HA 설계)와 닿고, **Mountkirk Games**는 글로벌 동시 출시·저지연 멀티플레이가 있어 멀티리전 GKE와 글로벌 상태 저장(Spanner)의 리전 장애 내성(DR 관점)과 닿는다. 두 케이스의 마이그레이션 서사는 EHR만큼 강하지 않으므로, 이 도메인 문항으로는 EHR·TerramEarth를 우선 대비하라.
...
**Q8. 컨테이너화하며 이전 (EHR Healthcare 유형)**

한 헬스케어 SaaS 기업이 온프렘의 레거시 애플리케이션을 GCP로 옮기려 한다. 요구는 "이전을 계기로 애플리케이션을 컨테이너화하고, 이후 오케스트레이션·자동 확장·롤링 업데이트로 운영하겠다"는 것이다. 가장 적합한 마이그레이션 전략과 도구는?

- (A) Migrate to Virtual Machines로 VM을 그대로 Compute Engine에 리호스트한다.
- (B) 컨테이너화하여 GKE로 이전하는 Replatform 전략을 택한다.
- (C) Transfer Appliance로 애플리케이션 디스크를 통째로 옮긴다.
- (D) 애플리케이션을 처음부터 서버리스로 완전 재작성하는 Refactor를 즉시 진행한다.
...
"이전을 계기로 컨테이너화 + 오케스트레이션/자동확장/롤링업데이트"는 전형적인 **Replatform** 신호다. VM을 그대로 두지 않고 컨테이너로 재포장해 **GKE**에 올리면 요구한 운영 특성을 얻는다. EHR Healthcare 케이스의 핵심 서사가 바로 이 "레거시 → 컨테이너 → GKE"다.

| 선택지 | 문제점 |
|--------|--------|
| (A) | Rehost는 VM을 그대로 옮길 뿐 컨테이너화·오케스트레이션 요구를 충족하지 못함 |
| (B) | 컨테이너화하며 GKE로 = Replatform. 요구와 정확히 일치 |
| (C) | 전송 도구는 데이터 이동용. 아키텍처 현대화와 무관 |
| (D) | 완전 재작성(Refactor)은 요구를 초과하고 시간·리스크가 과함. 지문은 "컨테이너화"까지만 요구 |

함정: "온프렘→GCP"만 보고 반사적으로 Rehost(A)를 고르지 말 것. "컨테이너화하며"가 있으면 Replatform이다.
...
**Q9. 점진 마이그레이션 중 온프렘 상시 연동**

수개월에 걸쳐 워크로드를 단계적으로 GCP로 옮기는 동안, 아직 이전하지 않은 온프렘 시스템과 이미 옮긴 GCP 워크로드가 **상시로 안정적·저지연** 통신해야 한다. 대역폭 요구가 크고 SLA가 중요하다. 가장 적합한 구성은?

- (A) Transfer Appliance를 주기적으로 배송받아 데이터를 동기화한다.
- (B) Storage Transfer Service로 온프렘 데이터를 GCS에 반복 복사한다.
- (C) Dedicated Interconnect(또는 요건에 따라 Partner Interconnect)로 하이브리드 연결을 구성한다.
- (D) 공용 인터넷으로 각 서비스가 직접 통신하게 둔다.
...
이 문제의 축은 **일회성 전송이 아니라 "상시 연동"**이다. 점진 마이그레이션 기간의 온프렘↔GCP 지속 통신은 하이브리드 연결의 일이고, 대역폭·저지연·SLA가 중요하면 **Dedicated Interconnect**(중간 규모·통신사 경유면 Partner Interconnect)가 정답이다. 상세 선택 기준은 [[/concept/cloud/03_vpc_for_pca]].

| 선택지 | 문제점 |
|--------|--------|
| (A) | Transfer Appliance는 대용량 **일회성 오프라인 전송**용. 상시 통신 수단이 아님 |
| (B) | STS는 데이터를 한 번/반복 **복사**하는 도구이지 시스템 간 상시 연동 경로가 아님 |
| (C) | 상시·저지연·고대역폭·SLA 하이브리드 연동 → Interconnect. 정답 |
| (D) | 공용 인터넷은 암호화·SLA·지연 보장이 없어 요구 위반 |

함정: **일시 전송(Transfer Appliance/STS)과 상시 연동(Interconnect/VPN)을 분리**하라. 이 둘을 뒤섞는 선택지가 단골 오답이다.
...
**Q10. 데이터 웨어하우스를 BigQuery로 (TerramEarth 유형)**

제조 기업이 기존 **Amazon Redshift** 데이터 웨어하우스와 여러 SaaS 소스의 데이터를 GCP의 BigQuery로 옮겨 분석을 통합하려 한다. 이전 후에도 소스에서 **주기적으로 증분 적재**가 필요하다. 가장 적합한 도구는?

- (A) Storage Transfer Service로 Redshift 데이터를 GCS로 옮긴 뒤 수동 로드한다.
- (B) Database Migration Service로 Redshift를 마이그레이션한다.
- (C) BigQuery Data Transfer Service로 소스를 BigQuery에 정기 적재하도록 구성한다.
- (D) Transfer Appliance로 웨어하우스 스냅샷을 오프라인 전송한다.
...
"타 DW(Redshift)·SaaS → **BigQuery로 정기/증분 적재**"는 **BigQuery Data Transfer Service(BQ DTS)**의 정의 그대로다. 파이프라인을 직접 짜지 않고 예약 주기로 반복 적재를 맡긴다.

| 선택지 | 문제점 |
|--------|--------|
| (A) | STS+수동 로드는 가능은 하나 정기·증분 적재를 직접 구현해야 함. 관리형 정기 적재는 BQ DTS |
| (B) | DMS는 **운영 트랜잭션 DB** 이전용. 분석 DW→BQ 적재 시나리오가 아님 |
| (C) | SaaS/Redshift/Teradata → BigQuery 정기 적재 = BQ DTS. 정답 |
| (D) | 오프라인 전송은 저대역폭 대용량 파일용. 정기 증분 적재 목적과 불일치 |

함정: **STS(범용 객체 전송) vs BQ DTS(BigQuery 분석 적재)**를 구분하라. "BigQuery로 정기 적재"가 나오면 BQ DTS다.
...
**Q11. 다수 워크로드의 백업 정책 중앙 관리**

한 기업이 수십 개의 Compute Engine VM과 데이터베이스에 대해 **일관된 백업 정책(주기·보존 기간)을 표준화하고, 보존·복구를 중앙에서 관리·자동화**하려 한다. 개별 팀이 스냅샷 스크립트를 제각각 운영하는 현 상태를 개선하려는 것이다. 가장 적합한 것은?

- (A) 각 VM에 cron으로 `gcloud` 스냅샷 스크립트를 배포한다.
- (B) Backup and DR Service로 백업 계획을 정의해 중앙에서 관리·복구한다.
- (C) Persistent Disk 스냅샷을 팀별로 수동 생성한다.
- (D) 모든 데이터를 Coldline 버킷에 복사해 두면 백업이 된다.
...
"다수 워크로드 + 정책 표준화 + 중앙 관리·복구 자동화"가 **Backup and DR Service**의 신호다. 개별 스냅샷을 손으로 돌리는 대신 백업 계획·보존·복구 오케스트레이션을 관리형으로 통합한다.

| 선택지 | 문제점 |
|--------|--------|
| (A) | 스크립트 분산은 정확히 이 문제가 개선하려는 현 상태. 표준화·중앙 관리가 안 됨 |
| (B) | 정책 기반 중앙 백업·복구 관리 = Backup and DR Service. 정답 |
| (C) | 수동 스냅샷은 팀별 편차·복구 오케스트레이션 부재로 요구 미충족 |
| (D) | 버킷 복사는 스토리지 계층 이동일 뿐, 백업 정책·정합성·복구 관리가 없음 |

함정: "디스크 하나 가끔 백업"이면 PD 스냅샷으로 충분하지만, **"여러 워크로드 정책 표준화·중앙 관리"**면 관리형 서비스다.
...
**Q12. 엔진을 바꾸는 이질 마이그레이션**

기업이 온프렘 **Oracle** 데이터베이스를 GCP의 **PostgreSQL(AlloyDB)**로 옮기려 한다. 라이선스 부담을 줄이려 엔진을 오픈소스 계열로 전환하는 것이 목적이다. 가장 정확한 설명과 도구는?

- (A) 동질 마이그레이션이므로 스키마 변환 없이 DMS 네이티브 복제만으로 끝난다.
- (B) 이질 마이그레이션이므로 DMS의 변환(conversion) 과정이 필요하고, 자동 변환되지 않는 객체는 수동 보정한다.
- (C) Migrate to Virtual Machines로 Oracle VM을 옮기면 자동으로 PostgreSQL이 된다.
- (D) Storage Transfer Service로 Oracle 데이터 파일을 복사하면 PostgreSQL에서 바로 읽힌다.
...
Oracle → PostgreSQL은 **다른 엔진**으로 옮기는 **이질(heterogeneous) 마이그레이션**이다. 데이터 타입·SQL 방언·스토어드 프로시저가 다르므로 **변환** 과정이 필요하고, 자동 변환되지 않는 객체(복잡한 PL/SQL 등)는 수동 보정이 따른다. DMS가 이 변환 워크스페이스를 제공한다.

| 선택지 | 문제점 |
|--------|--------|
| (A) | 엔진이 바뀌므로 동질이 아니다. 스키마·코드 변환이 필요함 |
| (B) | 이질 마이그레이션 = 변환 + 수동 보정. 정답 |
| (C) | Migrate to VMs는 VM을 그대로 옮길 뿐 DB 엔진을 바꾸지 않음 |
| (D) | 엔진이 다른 DB는 데이터 파일을 그대로 읽을 수 없음. 논리적 변환·복제 필요 |

함정: **동질(변환 불필요) vs 이질(변환 필요)**. "엔진을 바꾼다"가 나오면 이질이고, 변환·수동 보정 공수를 전제해야 한다.
...
**Q13. RTO 분 단위인데 완전 이중화는 부담**

전자상거래 백엔드의 DR 요구가 RTO 약 10분, RPO 수 분이다. 다만 예산상 **평상시 완전 규모의 이중 사이트(active-active)를 상시 가동하기는 부담**스럽다. 가장 적합한 DR 패턴은?

- (A) Backup & Restore(다른 리전에 백업만 보관)
- (B) Pilot Light(핵심 DB만 상시 복제, 컴퓨트는 꺼 둠)
- (C) Warm Standby(타 리전에 축소 규모로 상시 가동, 장애 시 스케일업)
- (D) Hot / Active-Active(양 리전 완전 규모 동시 운영)
...
"RTO 분 단위 + 완전 이중화는 부담"의 정답 신호는 **Warm Standby**다. 축소 규모지만 전체 스택이 상시 돌아가 장애 시 트래픽을 받고 스케일업하므로 RTO가 분 단위로 짧고, 완전 규모 active-active보다 평상시 비용이 낮다.

| 선택지 | 문제점 |
|--------|--------|
| (A) | Backup & Restore는 인프라를 새로 띄워야 해 RTO가 시간~일. 10분 요구 위반 |
| (B) | Pilot Light는 컴퓨트를 꺼 두므로 기동·스케일업에 시간이 더 걸려 분 단위 RTO에는 빠듯 |
| (C) | 축소 상시 가동 + 스케일업 = 분 단위 RTO, 완전 이중화보다 저비용. 정답 |
| (D) | active-active는 요구를 충족하지만 "완전 이중화 부담" 제약과 충돌(비용 최대) |

함정: **Pilot Light vs Warm Standby**의 경계 — 컴퓨트를 꺼 두면(Pilot Light) 기동 시간이 붙고, 축소라도 켜 두면(Warm Standby) 분 단위 복구가 가능하다. "완전 이중화는 부담"은 active-active를 배제하는 신호다.
