...
> "데이터는 이미 암호화되는데 왜 KMS를 또 쓰죠?", "IAM으로 권한을 막았는데 VPC-SC가 왜 필요하죠?" — PCA 보안 문제의 본질은 **레이어 구분**이다. 키를 누가 소유·통제하느냐(KMS),
> 비밀값을 어디에 두느냐(Secret Manager), 데이터가 경계 밖으로 나가는 것을 어떻게 막느냐(VPC-SC), 누가 어떤 맥락에서 앱에 접근하느냐(IAP), 그 모든 설정을 어떻게 한눈에 감시하느냐(SCC).
> 이 글은 IAM 권한 모델([[/concept/cloud/05_iam_for_pca]])과 **별개의 레이어**로 동작하는 GCP 보안 서비스들을 "요구사항 → 선택 기준 → 결론"으로 정리한다. PCA 준비 시리즈
> 9편이다.
...
GCP 보안 문제에서 가장 많이 틀리는 이유는 한 레이어로 다른 레이어의 일을 하려 들기 때문이다. IAM은 "누가 권한을 가지는가"를 정하지만, 그 권한을 가진 자가 **탈취된 자격증명으로 외부에서** 호출하는
것은 막지 못한다. 그것은 VPC-SC의 일이다. 데이터는 기본적으로 암호화되지만, 그 암호화 키를 **고객이 통제**해야 하는 컴플라이언스 요구는 KMS의 일이다.
...
이 글의 지도: 기본 암호화 → Cloud KMS(CMEK/CSEK/EKM) → Secret Manager → VPC-SC(+계층형 방화벽) → IAP(+Context-Aware Access) → <strong>Cloud Armor·엣지 방어(3레이어 혼동)</strong> → SCC → 소프트웨어 공급망 보안 → Shielded VM·Org Policy → <strong>컴플라이언스 설계(규제 매핑·데이터 주권·Sensitive Data Protection·인증·감사)</strong> → <strong>케이스 스터디 접점(EHR·TerramEarth)</strong> → 시험 공략 → 퀴즈. 각 축은 "요구사항 → 판단 기준 → 결론"으로 닫는다. IAM 역할·SA·리소스 계층은 <a href="#">05_iam_for_pca</a>에 있고 여기서는 중복 서술하지 않는다.
</div>
...
CMEK는 이 **KEK를 고객이 통제하는 KMS 키로 지정**하는 것이다. KEK는 KMS 밖으로 나오지 않으므로, 키를 비활성화하면 DEK를 풀 수 없고 → 데이터에 접근할 수 없게 된다. "키 폐기로 데이터를
즉시 접근 불가로 만들 수 있는가"라는 요구가 CMEK의 핵심 가치다.
...
대칭 키(symmetric encryption)는 **자동 로테이션 주기**를 설정할 수 있다(예: 90일마다). 로테이션이 일어나면 새 버전이 primary가 되어 이후 암호화는 새 버전으로, 기존 암호문 복호화는
해당 버전으로 처리된다. 비대칭 키(서명·비대칭 복호화)는 자동 로테이션을 지원하지 않아 수동 관리한다.
...
**결론**: CMEK = KMS에 둔 KEK를 고객이 관리. 키 링/키는 삭제 불가, 버전만 예약 폐기. 대칭 키만 자동 로테이션. HSM이 필요한지(FIPS Level 3), 외부 보관이 필요한지(EKM)는 규제
키워드로 판별.
...
Cloud External Key Manager(EKM)는 키를 **Google 외부의 서드파티 키 관리 파트너 시스템**에 두고, GCP는 외부 키로 암호화/복호화를 호출만 한다. 키 머티리얼은 외부 시스템을
떠나지 않는다.
...
**결론**: 비밀값은 Secret Manager에 — 코드·환경변수 하드코딩을 대체한다. 접근은 IAM(secretAccessor)으로 시크릿 단위 제어, 버전으로 로테이션·롤백, 데이터 거주는
user-managed 복제로. 키(KEK)는 KMS, 비밀값은 Secret Manager.
...
VPC Service Controls(VPC-SC)는 GCP **관리형 서비스(BigQuery·GCS 등)의 API 데이터 평면 주위에 서비스 경계(perimeter)**를 둘러, 경계 밖으로의 데이터 이동을
차단한다.
...
방화벽이 L3/L4를 제어한다는 점은 위 callout에서 정리했다. 그런데 방화벽 규칙을 **프로젝트마다 따로 관리**하면, 어떤 팀이 실수로 `0.0.0.0/0` SSH를 열어도 조직 차원에서 막을 방법이
없다. **계층형 방화벽 정책(Hierarchical Firewall Policy)**은 이 강제력을 **조직·폴더 레벨**로 끌어올린다.
...
평가는 **위에서 아래로** 진행되고, 상위에서 `allow`/`deny`로 결정나면 거기서 끝난다. 상위 정책은 하위가 덮어쓸 수 없다 — 조직 baseline을 **하위 프로젝트가 무력화하지 못하게** 강제하는
것이 핵심 가치다.
...
**결론**: VPC-SC = 관리형 서비스 데이터 평면 주위의 경계로 **데이터 유출(exfiltration) 방지**. IAM·방화벽과 다른 레이어. Access Level로 접근 조건,
ingress/egress로 통제된 교차 접근. 규제 데이터·자격증명 유출 대비가 키워드.
...
IAP는 Google의 **제로 트러스트** 모델(개념어 **BeyondCorp**)을 구현한다 — **네트워크 위치가 아니라 사용자 신원 + 컨텍스트(기기, IP, 시간)**로 매 요청을 인가한다. VPN 없이
인터넷 어디서든, 그러나 정책을 만족할 때만 애플리케이션·VM에 접근한다.
...
IAP가 **앱 앞단(L7)**에서 인증을 거는 것이라면, **Context-Aware Access(컨텍스트 인식 액세스)**는 같은 컨텍스트 조건을 **Google Cloud 콘솔·API·SaaS 앱 접근**
으로까지 넓힌 정책 모델이다. 기기 상태(회사 관리 기기 여부·암호화·OS 버전), 출발지 IP, 지역 같은 신호를 **Access Context Manager의 Access Level**로 정의하고, 그 레벨을
만족하지 않는 요청은 신원이 유효해도 막는다.
...
**결론**: IAP = 네트워크가 아닌 신원+컨텍스트 기반 접근(제로 트러스트 = BeyondCorp). 웹은 HTTPS LB 앞 L7, VM은 TCP forwarding으로 외부 IP·VPN 제거, 백엔드는
JWT 헤더 검증 필수. 같은 컨텍스트 조건을 콘솔·API까지 넓히면 **Context-Aware Access**(Chrome Enterprise Premium), 조건의 정의는 **Access Level**에
둔다.

---

## Cloud Armor — 엣지에서 트래픽을 거른다

### 무엇을 푸는가

IAP가 "**누가**(신원) 접근하는가"를 본다면, **Cloud Armor**는 "**무엇**(트래픽)이 들어오는가"를 본다. 외부에 공개된 애플리케이션 앞단(External Application Load
Balancer)에서 요청을 검사해 악성 트래픽·과도한 요청·L7 공격을 **엣지에서** 걸러낸다.

| 기능                      | 내용                                                                |
|-------------------------|-------------------------------------------------------------------|
| **DDoS 방어**             | L3/L4 볼류메트릭 공격 흡수(항상 켜진 기본 보호) + Adaptive Protection(ML 기반 이상 탐지) |
| **WAF 규칙**              | 사전 정의 규칙(SQLi·XSS·LFI·RCE 등 OWASP)으로 L7 공격 차단                     |
| **rate limiting**       | IP·헤더·지역 기준으로 요청 속도 제한(throttle) / 차단(ban)                        |
| **allow/deny/redirect** | IP 대역·지역(geo)·표현식(CEL)으로 허용·차단·리캡차 리다이렉트                          |
| **엣지 보안 정책**            | Cloud CDN 캐시 응답에도 적용                                              |

```
요구: "글로벌 공개 웹앱에 오는 SQLi·DDoS·특정 국가 트래픽을 앞단에서 차단"
→ Global External Application LB + Cloud Armor 보안 정책
→ preconfigured WAF 규칙 + geo/IP deny + rate-limit
```

### Cloud Armor와 IAP가 함께 걸릴 때 — 평가 순서

같은 External Application LB 백엔드에 **Cloud Armor와 IAP를 동시에** 붙일 수 있다. 이때 순서가 시험 포인트다.

```
요청 → External Application LB
  → ① Cloud Armor 보안 정책 평가
       ├ deny → 차단(IAP 평가 안 함)
       └ allow → ② IAP 평가(신원·컨텍스트)
                    ├ 불충족 → 차단
                    └ 충족 → 백엔드
```

**Cloud Armor가 먼저**다. 엣지에서 트래픽 위협을 거른 뒤, 통과한 요청에 대해서만 IAP가 신원을 확인한다. 즉 "봇/DDoS/악성 페이로드를 먼저 쳐내고, 살아남은 정상 요청의 사용자가 누구인지
IAP가 인가"하는 2단 게이트다.

### 세 레이어 혼동 — Cloud Armor vs IAP vs VPC-SC (시험 최다 함정)

보안 문제에서 가장 많이 뒤섞이는 세 서비스다. **무엇을 무엇으로부터 보호하는지**가 전부 다르다.
...
- 보호 대상: **외부 공개 앱**(ALB 앞단)
- 위협: DDoS·WAF(L7 공격)·악성/과다 트래픽
- 레이어: **엣지 트래픽**(L3/L4/L7)
- 신호어: "DDoS", "WAF", "rate-limit", "특정 국가 차단", "OWASP"
...
- 위협: 인가되지 않은 **사용자**의 접근
- 레이어: **신원+컨텍스트**(L7 앞단)
- 신호어: "VPN 없이", "신원 기반 접근", "외부 IP 없는 VM SSH", "제로 트러스트"
...
- 보호 대상: **관리형 서비스 데이터**(BigQuery·GCS API)
- 위협: 권한 있는 자격증명의 **데이터 유출(exfiltration)**
- 레이어: **데이터 평면 경계**
- 신호어: "유출 방지", "경계 밖 차단", "자격증명 유출돼도"

</div>
</div>

<div class="callout-warning">
시험 함정: 셋은 <strong>병렬로 함께 쓰는</strong> 것이지 택일이 아니다. "외부 공개 API를 DDoS·SQLi에서 보호" → <strong>Cloud Armor</strong>, "그 API에 누가 접근하는지 신원으로 통제" → <strong>IAP</strong>, "그 뒤 BigQuery 데이터가 경계 밖으로 새는 것 방지" → <strong>VPC-SC</strong>. 하나로 다른 둘의 일을 시키려는 보기가 오답이다. 특히 "DDoS를 VPC-SC로 막는다", "데이터 유출을 Cloud Armor로 막는다"는 전형적 함정 보기다.
</div>

**결론**: Cloud Armor = 엣지에서 트래픽 위협(DDoS·WAF·rate-limit)을 거르는 방어선. IAP·VPC-SC와는 보호 대상·레이어가 다르며, ALB 백엔드에서는 **Cloud Armor가
IAP보다 먼저 평가**된다.
...
지금까지의 서비스가 "실행 중인 자산"을 지켰다면, **공급망 보안은 그 자산이 어떻게 만들어져 배포되는가**를 지킨다. 컨테이너 이미지에 누군가 악성 코드를 심거나, 서명되지 않은 이미지가 프로덕션에 올라가거나,
취약한 OSS 의존성이 섞여 들어오는 것을 막는 레이어다.

### Binary Authorization — 신뢰된 이미지만 배포 허용

**Binary Authorization**은 배포 시점(deploy-time) 통제다. **GKE·Cloud Run**에 배포되는 컨테이너 이미지가 정책에서 요구하는 **서명(attestation)을 갖췄는지**
검증하고, 통과한 이미지만 실행을 허용한다.
...
**Artifact Analysis**(구 Container Analysis)는 **Artifact Registry**에 올라온 컨테이너 이미지·언어 패키지를 스캔해 알려진 취약점(CVE)을 찾는다. 푸시 시 자동
스캔(on-push)과, 이미 등록된 이미지에 대한 지속 분석(continuous analysis)을 제공한다. 이 스캔 결과를 Binary Authorization의 attestation 조건으로 연결하면 "취약점
있으면 배포 불가" 파이프라인이 된다.
...
**결론**: 공급망 보안 = "배포되는 코드를 신뢰할 수 있는가". Binary Authorization으로 서명된 이미지만 GKE/Cloud Run에 올리고, Artifact Analysis로 취약점을 걸러 그
결과를 attestation으로 연결하며, SLSA를 목표 등급으로 삼고, Assured OSS로 의존성 위험을 낮춘다.
...
> 참고로 **Confidential VM**은 다른 개념이다 — 사용 중(in-use) 메모리까지 암호화(AMD SEV 등)해 "처리 중 데이터" 기밀성을 보호한다. "부팅 무결성" → Shielded VM, "
> 메모리(사용 중) 암호화" → Confidential VM.
...
앞 절들이 "어떤 위협을 어느 레이어로 막는가"였다면, 컴플라이언스는 거꾸로 **"이 규제가 요구하는 통제를 어떤 GCP 기능으로 만족시키는가"**다. 시험에서 컴플라이언스 문제는 거의 항상 **규제 이름 →
통제 → 서비스** 매핑으로 나온다. 보안 서비스들을 다시 끌어오되, 이번엔 규제 요구가 출발점이다.
...
**데이터 거주(residency)**는 "데이터가 물리적으로 어느 리전에 저장되는가", **데이터 주권(sovereignty)**은 한발 더 나아가 "그 데이터에 대한 통제권(키·접근)이 특정 관할 밖으로 나가지
않는가"다.
...
**Assured Workloads**는 데이터 거주, 지원 인력의 접근 위치 제한, 규제별(예: FedRAMP, 특정 EU 요건) 통제를 **폴더 단위로 강제·검증**해 주는 제품이다. "리전 고정 + 인력 접근
통제 + 규제 준수 환경을 묶어서" 요구하면 개별 Org Policy 나열이 아니라 Assured Workloads가 답이다.
...
#### 키 통제 4종과 Assured Workloads — "키 소유"와 "규제 경계"를 가르는 축

암호화 문제는 결국 두 질문으로 갈린다 — **"키를 누가 저장·통제하는가"**(Google-managed → CMEK → CSEK/EKM 순으로 통제권이 고객에게 이동)와 **"규제 경계 자체를 강제하는가"**(
Assured Workloads). 앞 세 축이 키 메커니즘이라면 Assured Workloads는 키를 포함한 규제 환경 전체를 묶는 상위 개념이다.

| 서비스                   | 키를 누가 저장               | 키를 누가 관리                | 무엇을 위한 것                              | 신호어                             |
|-----------------------|------------------------|-------------------------|---------------------------------------|---------------------------------|
| **Google-managed**    | Google                 | Google                  | 기본 암호화(추가 작업 없음)                      | "데이터를 암호화"(단순)                  |
| **CMEK**              | **Google KMS**         | **고객**                  | 키 로테이션·폐기·감사 통제                       | "우리가 키를 로테이션·폐기·감사"             |
| **CSEK**              | Google 저장 안 함(메모리만)    | **고객**(매 요청 raw key 제공) | Google이 키를 보관조차 못하게(GCE·GCS 한정)       | "Google이 키를 저장하면 안 됨" + GCE/GCS |
| **Cloud EKM**         | **Google 외부** 서드파티 KMS | **고객**(외부 시스템)          | 데이터 주권·키 물리 분리                        | "키를 클라우드 밖 우리 시스템에"             |
| **Assured Workloads** | (키 아님) 리전·인력 접근·규제 패키지 | 고객이 정책, Google이 강제      | FedRAMP/IL4·5/CJIS/데이터 거주를 **묶어서** 강제 | "FedRAMP·정부·규제 경계를 한 번에"        |

<div class="callout-warning">
시험 함정: 이 넷을 같은 층으로 놓고 비교하는 보기가 오답 유발이다. CMEK·CSEK·EKM은 <strong>키 메커니즘</strong>(하나를 고른다)이고, Assured Workloads는 <strong>규제 경계 컨테이너</strong>(CMEK/EKM을 그 안에서 함께 쓴다). "FedRAMP 준수 환경을 만들라" → 단일 키 서비스가 아니라 <strong>Assured Workloads</strong>(폴더 단위 강제) + 그 안의 CMEK/EKM. "키를 Google이 저장조차 못하게" → 서비스 지원 범위를 보고 CSEK(GCE·GCS)인지 EKM(광범위·주권)인지 가른다.
</div>

### 상업적 민감정보 — Sensitive Data Protection

신용카드번호·주민번호·이메일 같은 **PII/민감정보**가 데이터셋 어디에 흩어져 있는지조차 모르는 것이 현실의 출발점이다. **Sensitive Data Protection**(구 Cloud DLP)은 이
데이터를 **탐지·분류·비식별(de-identify)**한다.
...
GCP는 **SOC 2 / SOC 1·3, ISO 27001(및 27017·27018), FedRAMP, PCI DSS, HIPAA** 등 다수의 인증·증명을 보유한다(증빙은 Compliance Reports
Manager에서 받는다). 하지만 인증은 **"인프라 계층"에 대한 것**이고, 그 위에 올린 워크로드의 설정·데이터·접근은 고객 몫이다.
...
기본 보존 기간은 로그 종류·버킷 설정에 따라 다르며(Admin Activity는 비교적 길게, Data Access는 짧게 보관되는 경향), 규제가 **장기 보존**을 요구하면 **로그 싱크(sink)**로
내보낸다.
...
**결론(컴플라이언스)**: 규제는 통제 목표를 주고, 우리가 서비스로 번역한다 — 거주는 `resourceLocations`/Assured Workloads, 주권은 EKM, 민감정보는 Sensitive Data
Protection(탐지·de-id), 증명은 Cloud Audit Logs(특히 Data Access는 직접 켠다)·로그 싱크·Access Transparency. 인증은 인프라 계층 보증일 뿐,
설정·데이터·접근은 공동 책임의 고객 몫이다.
...
- **Regulation**(HIPAA·GDPR·PCI DSS 등) —`requires`→ **Control**(암호화·경계·감사·de-id): 규제 노드가 통제 목표를 요구하고, 통제는 다시 위 서비스 노드로
  `satisfied_by` 연결된다.
- **Sensitive Data Protection** —`classifies`→ **Dataset/Column**: 민감 데이터 패턴을 데이터 자산에 라벨링하는 행위, de-identify는
  `transforms`로 표현.
- **Attestation**(Binary Authorization) —`vouches_for`→ **Image** —`deployed_to`→ **GKE/Cloud Run**: 공급망 무결성을 빌드→배포 경로의
  엣지로 표현.
- **Audit Log** —`records`→ **(Principal, Action, Resource)**: 감사 로그는 IAM 그래프의 행위를 시간축으로 관측한 이벤트 노드.

즉 "키–비밀–경계–조건"은 데이터 자산 노드 주위를 감싸는 **거버넌스 레이어 서브그래프**를 이루고, "규제–통제–서비스"와 "이미지–서명–배포"가 그 위에 겹친다.
IAM([[/concept/cloud/05_iam_for_pca]])의 "Principal–Role–Resource" 그래프와 직교(orthogonal)하게 교차한다.

---

## 케이스 스터디 접점 — 보안 요구를 설계 결정으로

PCA 시험은 4개 공식 케이스 스터디(EHR Healthcare·Helicopter Racing League·Mountkirk Games·TerramEarth)에서 케이스당 여러 문항을 낸다. 보안 관점에서 직접
맞닿는 것은 **EHR Healthcare**(HIPAA·하이브리드)와 **TerramEarth**(파트너 API·데이터 반출)다. 아래 매핑은 케이스 요구 → 이 글에서 다룬 통제로 잇는다.

<div class="callout-note">
케이스 세부 수치(병원 수·차량 대수 등)는 시험 버전(현행 v6.1)에 따라 달라질 수 있으므로 여기서는 <strong>요구사항 → 통제 매핑</strong>에 집중한다. 아래 요구 요약은 공개 케이스 개요 기준이며, 응시 전 최신 케이스 원문으로 재확인할 것.
</div>

### EHR Healthcare — HIPAA·하이브리드 보안 (핵심 케이스)

멀티병원 EHR SaaS, HIPAA 필수, 온프렘 레거시와 연동, 전 접근 감사, 저장·전송 암호화가 요구된다. 보안 설계는 여러 레이어의 조합이다.

| 케이스 요구                    | 보안 통제                                                                                                             | 이 글의 절    |
|---------------------------|-------------------------------------------------------------------------------------------------------------------|-----------|
| HIPAA — 저장 데이터 암호화 + 키 통제 | **CMEK**(BAA 체결 전제, 키 로테이션·폐기·감사)                                                                                 | Cloud KMS |
| PHI가 경계 밖으로 유출되지 않게       | **VPC Service Controls**(BigQuery·GCS 데이터 평면 경계)                                                                  | VPC-SC    |
| 온프렘 레거시와 안전한 저지연 연동       | **Dedicated Interconnect**(전용 회선, 전송 암호화 병행) — 자세히는 [[/concept/cloud/07_load_balancing_and_connectivity_for_pca]] | 전송 보안     |
| 모든 접근을 감사·증빙              | **Cloud Audit Logs**(특히 Data Access 로그 활성화) + 로그 싱크 장기 보존                                                         | 감사        |
| 내부 관리 콘솔을 VPN 없이 안전하게     | **IAP** + Context-Aware Access(기기·위치 조건)                                                                          | IAP       |
| 규제 준수 여부를 조직 차원에서 상시 점검   | **SCC Premium**(설정 오류·컴플라이언스 리포트)                                                                                 | SCC       |
| 민감 필드(환자 식별자) 분석 시 비식별    | **Sensitive Data Protection**(탐지·de-identify)                                                                     | 컴플라이언스    |

<div class="callout-warning">
EHR 함정: "HIPAA를 만족하라"에 단일 서비스를 고르면 틀린다. HIPAA는 <strong>BAA 체결 + 최소권한 IAM + CMEK + VPC-SC + 감사 로그</strong>의 조합이다. 또한 GCP가 HIPAA를 "지원"한다는 것은 <strong>고객이 통제를 올바로 구성했을 때</strong> 성립한다(공동 책임). "GCP가 HIPAA 인증이니 자동 준수"는 오답 보기다.
</div>

### TerramEarth — 파트너 API 보안·데이터 반출

500+ 딜러가 쓰는 API, IoT 텔레메트리 수집, 예지정비 데이터가 있다. 보안 관점에서는 **외부 파트너에게 API를 안전하게 노출**하고, **통제된 데이터 반출**을 어떻게 설계하느냐가 핵심이다.

| 케이스 요구                               | 보안 통제                                                                                   | 판단                           |
|--------------------------------------|-----------------------------------------------------------------------------------------|------------------------------|
| 딜러(외부 파트너)에게 API를 안전하게 노출            | **Apigee**(API 게이트웨이 — 인증·쿼터·분석) + 공개 엔드포인트는 **Cloud Armor**(WAF·rate-limit)            | API 관리는 Apigee, 엣지 위협은 Armor |
| 파트너 API 앞단 DDoS·악성 트래픽 차단            | **Cloud Armor** 보안 정책(rate-limit·geo·WAF)                                               | 엣지 방어                        |
| 파트너 프로젝트로의 **통제된** 데이터 공유(무단 반출은 차단) | **VPC-SC egress 규칙**(허용된 파트너 프로젝트만 반출 허용)                                               | 데이터 경계                       |
| 파트너 인증에 SA 키를 뿌리지 않기                 | **Workload Identity Federation**(외부 워크로드→GCP, 키 없이) — [[/concept/cloud/05_iam_for_pca]] | IAM 연계                       |

<div class="callout-warning">
TerramEarth 함정: "파트너에게 데이터를 공유하되 무단 반출은 막아라"는 VPC-SC를 <strong>완전 차단</strong>으로 오해하기 쉽다. 정답은 VPC-SC <strong>egress 규칙</strong>으로 "허용된 파트너 프로젝트로의 반출만" 선별 허용하는 것이다. 또 "파트너 앱이 SA 키로 접근"은 키 유출 위험 — <strong>Workload Identity Federation</strong>으로 키 없는 단기 자격증명을 쓴다.
</div>

**결론(케이스)**: 보안 케이스 문제는 "규제·파트너 요구 → 여러 레이어의 조합"으로 푼다. EHR은 HIPAA=CMEK+VPC-SC+감사+IAP의 묶음, TerramEarth는 파트너
API=Apigee+Cloud Armor+VPC-SC egress+Workload Identity Federation의 묶음이다. 단일 서비스로 답하는 보기를 경계한다.
...
| "외부 공개 앱 DDoS·WAF·L7 rate-limit·국가 차단" | **Cloud Armor**                                     | 엣지 방어      |
| "Cloud Armor와 IAP를 같이 붙인 ALB 평가 순서"    | Armor 먼저 → 통과 시 IAP                                 | 엣지+앱 접근    |
| "파트너 API를 안전하게 노출(인증·쿼터)"              | **Apigee**(+엣지는 Cloud Armor)                        | API 관리     |
| "허용된 파트너 프로젝트로만 데이터 반출"                | **VPC-SC egress 규칙**                                | 데이터 경계     |
...
| Cloud Armor vs IAP vs VPC-SC                    | Armor=엣지 트래픽 위협(DDoS·WAF·rate-limit) / IAP=신원 기반 앱 접근 / VPC-SC=데이터 유출 경계. 보호 대상이 셋 다 다름, 병렬 사용 |
| Cloud Armor vs 방화벽                              | Armor=L7 애플리케이션 위협(ALB 앞단, WAF·DDoS) / 방화벽=L3/L4 네트워크 트래픽(VM 간)                                |
| Armor+IAP 순서                                    | 같은 ALB 백엔드에서 **Cloud Armor 먼저** 평가 → deny면 IAP 평가 안 함, allow면 IAP가 신원 인가                       |
...
금융 규제 기관이 요구한다 — "암호화 키를 클라우드 제공자(Google)가 어떤 형태로도 저장·보유해서는 안 되며, 키는 우리가 운영하는 키 관리 시스템에 둬야 한다." BigQuery에 저장된 데이터를 이
조건으로 보호해야 한다. 올바른 선택은?
...
Cloud KMS에서 **Key Ring과 Key는 삭제할 수 없다**(이름이 영구 점유됨). location은 생성 후 **변경 불가**다. 정리할 수 있는 단위는 Key **Version**이며, 그것도 예약
폐기(scheduled for destruction) 상태로 대기 후 파기된다.
...
PII를 담은 BigQuery 데이터셋이 있다. 보안팀의 우려 — "내부자 또는 유출된 SA 키를 가진 공격자가, **유효한 IAM 권한을 그대로 가진 채** 회사 외부에서 데이터를 자신의 GCS 버킷으로 반출할 수
있다." IAM 권한 설정은 이미 최소 권한으로 되어 있다. 추가로 필요한 것은?
...
핵심은 "**유효한 IAM 권한을 가진 채**"라는 문구다 — IAM은 권한이 있으니 통과시킨다. 이 유출(exfiltration)을 막는 것은 IAM과 별개 레이어인 **VPC-SC**다. 경계를 만들고
Access Level로 신뢰 네트워크에서만 접근을 허용하면, 외부에서 온 요청은 권한이 있어도 차단된다.
...
레거시 내부 웹 애플리케이션(GCE 인스턴스 그룹에서 구동)을 직원들이 재택에서 접근해야 한다. 요구 — VPN을 쓰지 않고, 회사가 관리하는 신원으로만 접근하며, 향후 기기·지역 조건을 추가할 수 있어야 한다.
가장 적합한 설계는?
...
"VPN 없이, 신원 기반, 컨텍스트 확장 가능" = **IAP for Web**(BeyondCorp). 웹 앱이므로 **HTTPS 부하 분산기(L7) 앞단**에서 IAP가 인증하고, IAM 역할(
`iap.httpsResourceAccessor`)로 접근을 제어한다. 기기·지역 조건은 Access Context Manager의 Access Level로 추가한다.
...
조직이 다음을 원한다 — ① 실시간 위협 탐지(악성 활동·비정상 행위), ② CIS·PCI DSS 컴플라이언스 대시보드, ③ 대상은 GCP 단일 클라우드. 비용은 합리적 범위에서. 어떤 SCC 티어가 최소 요구를
충족하는가?
...
위협 탐지(Event/Container/VM Threat Detection)와 컴플라이언스 리포트(CIS·PCI 등)는 **Premium 이상**에서 제공된다. 대상이 GCP 단일 클라우드이므로
멀티클라우드·SIEM/SOAR가 필요한 **Enterprise까지는 불필요**하다 — "비용 합리적" 요구를 고려하면 과잉이다.
...
"서명된 이미지만 배포, 미서명 배포 차단" = 배포 시점 게이트인 **Binary Authorization**. 취약점 스캔(Artifact Analysis) 결과를 attestation으로 연결하면 "스캔
통과 → 서명 → 배포 허용" 파이프라인이 된다.
...
결제 서비스가 PCI DSS 범위에 든다. 분석팀은 거래 데이터로 리포트를 만들어야 하지만 **원본 카드번호(PAN)가 분석 환경에 그대로 노출되면 안 된다**. 단, 결제 정산 단계에서는 원래 값으로 **되돌릴 수
있어야** 한다. 가장 적합한 설계는?
...
핵심 단서는 "**되돌릴 수 있어야 한다**"이다. 비식별 기법 중 **토큰화(형식보존 암호화)는 가역적**이라 키로 원본을 복원할 수 있어 정산 단계에 부합한다. 탐지·분류·de-id는 **Sensitive
Data Protection(구 Cloud DLP)**의 일이다.
...
데이터 **읽기/쓰기**는 **Data Access 감사 로그**가 기록하며, 이 로그는 4종 중 유일하게 **기본 비활성**이라 명시적으로 켜야 한다. 장기·변조 불가 보존은 **로그 싱크 → Cloud
Storage(버킷 잠금)**가 정석이다.
...
**Q9. 세 레이어 혼동 — DDoS·WAF는 누구 일인가**

글로벌 공개 전자상거래 API가 있다. 요구 — ① 대규모 DDoS와 SQL 인젝션 같은 L7 공격을 앞단에서 차단, ② 특정 국가의 트래픽 차단, ③ IP당 요청 속도 제한. 어떤 서비스로 구현하는가?

- (A) VPC Service Controls로 API 주위에 서비스 경계를 만든다.
- (B) Cloud Armor 보안 정책 — preconfigured WAF 규칙 + geo/IP deny + rate limiting.
- (C) Identity-Aware Proxy로 신원 기반 접근을 강제한다.
- (D) 방화벽 규칙으로 악성 IP를 차단한다.
...
DDoS·WAF(L7 공격)·rate-limit·geo 차단은 엣지 트래픽 방어인 **Cloud Armor**의 일이다. External Application LB 앞단에서 보안 정책으로 처리한다.

| 선택지 | 문제점                                                          |
|-----|--------------------------------------------------------------|
| (A) | VPC-SC는 관리형 서비스 **데이터 유출** 경계 — 들어오는 DDoS·WAF 트래픽을 막는 도구가 아님 |
| (C) | IAP는 **신원** 기반 접근 게이트 — DDoS·SQLi 같은 트래픽 위협을 다루지 않음          |
| (D) | 방화벽은 L3/L4 — SQLi(L7 페이로드)·정교한 rate-limit·geo 정책을 처리하지 못함    |

세 서비스(Armor/IAP/VPC-SC)는 보호 대상이 각각 다르며 실제로는 병렬로 함께 쓴다.
...
**Q10. Cloud Armor와 IAP 평가 순서**

같은 External Application LB 백엔드에 Cloud Armor(악성 트래픽 차단)와 IAP(직원 신원 인증)를 모두 붙였다. 어떤 IP에서 온 요청이 Cloud Armor의 deny 규칙에 걸린다.
이 요청은 어떻게 처리되는가?

- (A) IAP가 먼저 신원을 확인하고, 인증되면 Cloud Armor는 통과시킨다.
- (B) Cloud Armor가 먼저 평가되어 deny되므로, IAP 평가까지 가지 않고 차단된다.
- (C) 두 정책이 동시에 평가되어 하나라도 allow면 통과한다.
- (D) IAP를 붙이면 Cloud Armor는 비활성화된다.
...
External Application LB 백엔드에서는 **Cloud Armor가 IAP보다 먼저** 평가된다. Cloud Armor에서 deny되면 그 지점에서 차단되고 IAP 평가는 일어나지 않는다. allow된
요청만 IAP가 신원·컨텍스트를 인가한다("트래픽 먼저 거르고, 살아남은 요청의 사용자를 인가").

| 선택지 | 문제점                                      |
|-----|------------------------------------------|
| (A) | 순서가 반대 — 트래픽 방어(Armor)가 신원 인증(IAP)보다 앞선다 |
| (C) | 둘은 OR가 아니라 순차 게이트 — 둘 다 통과해야 백엔드에 도달     |
| (D) | 둘은 독립적으로 함께 동작한다                         |
...
**Q11. EHR Healthcare — HIPAA 설계 조합**

병원 EHR SaaS를 GCP로 이전한다. HIPAA 준수가 필수이며 ① 저장 데이터의 키를 병원이 로테이션·폐기할 수 있어야 하고, ② PHI가 신뢰 경계 밖으로 유출되지 않아야 하며, ③ 데이터에 누가 접근했는지
전부 감사해야 한다. 가장 적합한 조합은?

- (A) Google-managed 암호화 하나로 HIPAA가 자동 준수된다.
- (B) CMEK(키 통제) + VPC Service Controls(유출 경계) + Data Access 감사 로그 활성화 + BAA 체결.
- (C) CSEK로 BigQuery 데이터를 암호화하고 방화벽으로 외부 IP를 막는다.
- (D) Cloud Armor로 병원 앱을 보호하면 HIPAA가 충족된다.
...
HIPAA는 단일 서비스가 아니라 **여러 레이어의 조합**이다 — 키 통제(**CMEK**), 유출 방지 경계(**VPC-SC**), 접근 감사(**Data Access 로그** 활성화), 그리고 법적 전제인 *
*BAA 체결**. GCP의 HIPAA "지원"은 고객이 통제를 올바로 구성했을 때 성립한다(공동 책임).

| 선택지 | 문제점                                                          |
|-----|--------------------------------------------------------------|
| (A) | 키를 Google이 통제 — "병원이 키를 로테이션·폐기" 요구 미충족. 인증이 자동 준수를 뜻하지도 않음  |
| (C) | **BigQuery는 CSEK 미지원**(CMEK를 씀). 방화벽(L3/L4)은 데이터 평면 유출을 못 막음 |
| (D) | Cloud Armor는 엣지 트래픽 방어 — 키 통제·유출 경계·감사와 무관                   |
...
**Q12. TerramEarth — 파트너 데이터 반출 통제**

중장비 제조사가 딜러(외부 파트너)와 텔레메트리 데이터를 공유한다. 요구 — 승인된 특정 파트너 프로젝트로의 데이터 반출은 허용하되, 그 외 모든 경계 밖 반출은 차단해야 한다. 어떻게 설계하는가?

- (A) VPC Service Controls로 경계를 만들되, 모든 egress를 완전 차단한다.
- (B) VPC Service Controls 경계 + **egress 규칙**으로 승인된 파트너 프로젝트로의 반출만 선별 허용한다.
- (C) IAM에서 파트너에게 Viewer 역할만 부여하면 충분하다.
- (D) Cloud Armor로 파트너 IP만 allow한다.
...
"승인된 대상으로만 반출 허용, 나머지 차단"은 VPC-SC의 **egress 규칙**으로 통제된 반출을 구성하는 문제다. 경계는 기본적으로 밖으로의 접근을 막고, egress 규칙이 특정 파트너 프로젝트·서비스로의
예외를 연다.

| 선택지 | 문제점                                                        |
|-----|------------------------------------------------------------|
| (A) | 완전 차단은 "승인된 파트너로는 공유해야 한다" 요구에 위배 — egress 규칙으로 선별 허용해야 함  |
| (C) | IAM Viewer는 권한만 부여 — 권한 있는 자격증명의 경계 밖 반출을 못 막음(VPC-SC의 영역) |
| (D) | Cloud Armor는 **들어오는** 엣지 트래픽 방어 — 나가는 데이터 반출 경계가 아님        |

보너스: 파트너 앱 인증에는 SA 키 배포 대신 **Workload Identity Federation**으로 키 없는 단기 자격증명을 쓴다.
...
**Q13. 규제 경계 — FedRAMP 준수 환경**

정부 계약 워크로드가 **FedRAMP** 준수 환경을 요구한다 — 데이터는 미국 리전에만 저장, Google 지원 인력의 접근은 미국 내 인가된 인력으로 제한, 규제 통제가 폴더 단위로 강제·검증되어야 한다. 가장
적합한 것은?

- (A) Org Policy `resourceLocations`로 리전만 고정하면 FedRAMP가 충족된다.
- (B) Cloud EKM으로 키를 외부에 두면 FedRAMP가 충족된다.
- (C) Assured Workloads로 규제 패키지(리전 제약 + 인력 접근 통제 + 통제 강제)를 폴더 단위로 적용한다.
- (D) VPC Service Controls 경계 하나로 FedRAMP를 만족한다.
...
"데이터 거주 + 인력 접근 제한 + 규제(FedRAMP) 통제를 묶어서 강제"는 개별 Org Policy 나열이 아니라 **Assured Workloads**의 영역이다. 폴더 단위로 규제 경계를 강제·검증하고,
그 안에서 CMEK/EKM 등을 함께 쓴다.

| 선택지 | 문제점                                                  |
|-----|------------------------------------------------------|
| (A) | `resourceLocations`는 리전 고정만 — 인력 접근 제한·규제 패키지 강제는 못함 |
| (B) | EKM은 키 주권 한 축일 뿐 — FedRAMP 환경 전체를 구성하지 않음            |
| (D) | VPC-SC는 데이터 유출 경계 — 리전·인력·규제 패키지 강제와 다른 레이어          |

CMEK·CSEK·EKM은 **키 메커니즘**, Assured Workloads는 그들을 감싸는 **규제 경계 컨테이너**임을 구분한다.
...
**Q14. 워크로드 무결성 — 처리 중 데이터 기밀성**

규제 요구 — 데이터가 **메모리에서 처리되는 동안(in-use)**에도 암호화되어, 하이퍼바이저나 클라우드 운영자조차 평문 메모리를 볼 수 없어야 한다. 어떤 것을 선택하는가?

- (A) Shielded VM — Secure Boot·vTPM·Integrity Monitoring.
- (B) Confidential VM — 사용 중(in-use) 메모리를 하드웨어(AMD SEV 등)로 암호화한다.
- (C) CMEK로 디스크를 암호화한다.
- (D) VPC Service Controls로 경계를 만든다.
...
"사용 중(메모리) 데이터 암호화, 운영자도 평문을 못 봄" = **Confidential VM**(AMD SEV 등으로 메모리 암호화). "처리 중 데이터(in-use)" 기밀성이 신호어다.

| 선택지 | 문제점                                               |
|-----|---------------------------------------------------|
| (A) | Shielded VM은 **부팅** 무결성(루트킷·부트킷 방지) — 메모리 암호화가 아님 |
| (C) | CMEK는 **저장(at-rest)** 데이터 키 통제 — 처리 중 메모리와 무관     |
| (D) | VPC-SC는 데이터 유출 경계 — 무결성/기밀성 레이어가 아님               |

암기: **at-rest = CMEK/CSEK/EKM, in-transit = TLS, in-use(메모리) = Confidential VM**. 부팅 무결성은 Shielded VM으로 별개.
...
처음의 두 질문에 답한다. "데이터는 이미 암호화되는데 왜 KMS?" — 기본 암호화는 키를 Google이 통제한다. CMEK/CSEK/EKM은 **키 통제권을 고객에게 옮기는 정도의 차이**다. "IAM으로
막았는데 왜 VPC-SC?" — IAM은 권한, VPC-SC는 **경계**다. 권한 있는 자격증명이 경계 밖에서 데이터를 빼가는 것은 다른 레이어가 막는다.
...
- Google Cloud, [*Customer-supplied encryption keys (
  CSEK)*](https://cloud.google.com/compute/docs/disks/customer-supplied-encryption) — CSEK 지원 범위·책임 경계
...
- Google Cloud, [*VPC Service Controls*](https://cloud.google.com/vpc-service-controls/docs/overview) — 서비스 경계·Access
  Level·ingress/egress
- Google Cloud, [*Identity-Aware Proxy*](https://cloud.google.com/iap/docs/concepts-overview) — IAP for Web·TCP
  forwarding, BeyondCorp
- Google Cloud, [*Security Command Center*](https://cloud.google.com/security-command-center/docs) —
  Standard·Premium·Enterprise 티어
- Google Cloud, [*Shielded VM*](https://cloud.google.com/security/products/shielded-vm) — Secure Boot·vTPM·Integrity
  Monitoring
- Google Cloud, [*Hierarchical firewall policies*](https://cloud.google.com/firewall/docs/firewall-policies-overview) —
  조직·폴더 레벨 강제, goto_next 평가
- Google Cloud, [*Binary Authorization*](https://cloud.google.com/binary-authorization/docs) — 배포 시점 서명 검증,
...
- Google Cloud, [*Context-aware access*](https://cloud.google.com/beyondcorp-enterprise/docs/context-aware-access) —
  Access Level 기반 접근, Chrome Enterprise Premium
- Google Cloud, [*Sensitive Data Protection (구 Cloud DLP)*](https://cloud.google.com/sensitive-data-protection/docs) —
  탐지·분류·de-identify(마스킹·토큰화)
- Google Cloud, [*Assured Workloads*](https://cloud.google.com/assured-workloads/docs) — 데이터 거주·인력 접근 제한·규제 패키지
- Google Cloud, [*Cloud Audit Logs*](https://cloud.google.com/logging/docs/audit) — Admin Activity·Data Access·System
  Event·Policy Denied
- Google Cloud, [*Access Transparency*](https://cloud.google.com/assured-workloads/access-transparency/docs) — Google 인력
  접근 로그(+Access Approval)
- Google Cloud, [*Compliance offerings*](https://cloud.google.com/security/compliance) — SOC·ISO·FedRAMP·PCI 인증, 공동 책임
  모델
