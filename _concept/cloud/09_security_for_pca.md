...
tag     : cloud gcp security kms vpcsc iap armor compliance pca
toc     : true
comment : true
latex   : false
status  : complete
...
IAP가 "**누가**(신원) 접근하는가"를 본다면, **Cloud Armor**는 "**무엇**(트래픽)이 들어오는가"를 본다. 외부에 공개된 애플리케이션 앞단(External Application Load Balancer)에서 요청을 검사해 악성 트래픽·과도한 요청·L7 공격을 **엣지에서** 걸러낸다.
...
**Cloud Armor가 먼저**다. 엣지에서 트래픽 위협을 거른 뒤, 통과한 요청에 대해서만 IAP가 신원을 확인한다. 즉 "봇/DDoS/악성 페이로드를 먼저 쳐내고, 살아남은 정상 요청의 사용자가 누구인지 IAP가 인가"하는 2단 게이트다.
...
**결론**: Cloud Armor = 엣지에서 트래픽 위협(DDoS·WAF·rate-limit)을 거르는 방어선. IAP·VPC-SC와는 보호 대상·레이어가 다르며, ALB 백엔드에서는 **Cloud Armor가 IAP보다 먼저 평가**된다.
...
암호화 문제는 결국 두 질문으로 갈린다 — **"키를 누가 저장·통제하는가"**(Google-managed → CMEK → CSEK/EKM 순으로 통제권이 고객에게 이동)와 **"규제 경계 자체를 강제하는가"**(Assured Workloads). 앞 세 축이 키 메커니즘이라면 Assured Workloads는 키를 포함한 규제 환경 전체를 묶는 상위 개념이다.
...
PCA 시험은 4개 공식 케이스 스터디(EHR Healthcare·Helicopter Racing League·Mountkirk Games·TerramEarth)에서 케이스당 여러 문항을 낸다. 보안 관점에서 직접 맞닿는 것은 **EHR Healthcare**(HIPAA·하이브리드)와 **TerramEarth**(파트너 API·데이터 반출)다. 아래 매핑은 케이스 요구 → 이 글에서 다룬 통제로 잇는다.
...
**결론(케이스)**: 보안 케이스 문제는 "규제·파트너 요구 → 여러 레이어의 조합"으로 푼다. EHR은 HIPAA=CMEK+VPC-SC+감사+IAP의 묶음, TerramEarth는 파트너 API=Apigee+Cloud Armor+VPC-SC egress+Workload Identity Federation의 묶음이다. 단일 서비스로 답하는 보기를 경계한다.
...
같은 External Application LB 백엔드에 Cloud Armor(악성 트래픽 차단)와 IAP(직원 신원 인증)를 모두 붙였다. 어떤 IP에서 온 요청이 Cloud Armor의 deny 규칙에 걸린다. 이 요청은 어떻게 처리되는가?
...
External Application LB 백엔드에서는 **Cloud Armor가 IAP보다 먼저** 평가된다. Cloud Armor에서 deny되면 그 지점에서 차단되고 IAP 평가는 일어나지 않는다. allow된 요청만 IAP가 신원·컨텍스트를 인가한다("트래픽 먼저 거르고, 살아남은 요청의 사용자를 인가").
...
병원 EHR SaaS를 GCP로 이전한다. HIPAA 준수가 필수이며 ① 저장 데이터의 키를 병원이 로테이션·폐기할 수 있어야 하고, ② PHI가 신뢰 경계 밖으로 유출되지 않아야 하며, ③ 데이터에 누가 접근했는지 전부 감사해야 한다. 가장 적합한 조합은?
...
HIPAA는 단일 서비스가 아니라 **여러 레이어의 조합**이다 — 키 통제(**CMEK**), 유출 방지 경계(**VPC-SC**), 접근 감사(**Data Access 로그** 활성화), 그리고 법적 전제인 **BAA 체결**. GCP의 HIPAA "지원"은 고객이 통제를 올바로 구성했을 때 성립한다(공동 책임).
...
"승인된 대상으로만 반출 허용, 나머지 차단"은 VPC-SC의 **egress 규칙**으로 통제된 반출을 구성하는 문제다. 경계는 기본적으로 밖으로의 접근을 막고, egress 규칙이 특정 파트너 프로젝트·서비스로의 예외를 연다.
...
정부 계약 워크로드가 **FedRAMP** 준수 환경을 요구한다 — 데이터는 미국 리전에만 저장, Google 지원 인력의 접근은 미국 내 인가된 인력으로 제한, 규제 통제가 폴더 단위로 강제·검증되어야 한다. 가장 적합한 것은?
...
"데이터 거주 + 인력 접근 제한 + 규제(FedRAMP) 통제를 묶어서 강제"는 개별 Org Policy 나열이 아니라 **Assured Workloads**의 영역이다. 폴더 단위로 규제 경계를 강제·검증하고, 그 안에서 CMEK/EKM 등을 함께 쓴다.
