...
### 명칭 현행화 — 구명칭이 문제 지문에 섞여 나온다

Google은 2022년 LB 제품군을 **Application/Network × global/regional/classic × proxy/passthrough** 체계로 리브랜딩했고 이후 대규모 재명명은 없었다. 다만 시험 문항·기존 문서·강의에는 **구명칭이 여전히 등장**하므로, 구/신 명칭 매핑을 손에 쥐고 있어야 지문을 오독하지 않는다.

| 구명칭(레거시) | 현행 명칭 | 비고 |
|----------------|-----------|------|
| **External HTTP(S) Load Balancing** (global) | **Global External Application LB** | L7, 글로벌 anycast, GFE 기반 |
| **External HTTP(S) Load Balancing** (classic) | **Classic Application LB** | 구 global HTTP(S) LB의 classic 모드. 신규는 global 권장 |
| Regional HTTP(S) Load Balancing | **Regional External Application LB** | Envoy 기반, 리전 고정(데이터 거주) |
| Internal HTTP(S) Load Balancing | **Internal Application LB** | VPC 내부 L7 |
| **TCP/SSL Proxy Load Balancing** | **Global External Proxy Network LB** | L4 프록시, 비HTTP TCP에 글로벌 IP |
| **Network Load Balancing** (target pool 기반) | **External Passthrough Network LB** | L4 패스스루, 클라이언트 IP 보존, 리전 전용 |
| Internal TCP/UDP Load Balancing | **Internal Passthrough Network LB** | 내부 L4, 커스텀 라우트 next hop |

<div class="callout-warning">
지문에 "<strong>HTTP(S) Load Balancer</strong>"가 보이면 대부분 <strong>Global External Application LB</strong>를 가리키고, "<strong>TCP Proxy / SSL Proxy</strong>"는 <strong>Proxy Network LB</strong>, 옛 "<strong>Network Load Balancing</strong>"(target pool)은 <strong>Passthrough Network LB</strong>다. 구명칭 "Network Load Balancing"을 지금의 포괄어 "Network LB"(= L4 전체)와 혼동하면 패스스루/프록시 구분에서 실수한다. 명칭이 아니라 <strong>4축 속성</strong>(글로벌/외부/L7/프록시)으로 되짚어라.
</div>
...
### 결정트리 — 세 개의 분기로 6종이 좁혀진다

지문에서 **① 노출 방향(외부/내부) → ② 계층(L7/L4) → ③ 범위·모드(글로벌/리전, 프록시/패스스루)** 순서로 물으면 후보가 하나로 수렴한다. 순서가 중요하다 — L7/L4를 먼저 정하면 CDN·WAF 결합 가능 여부가 그 자리에서 갈리기 때문이다.

```mermaid
flowchart TD
  Q0["트래픽 진입점을 고른다"] --> Q1{"외부(인터넷)인가<br/>내부(VPC)인가?"}
  Q1 -->|외부| E1{"HTTP/HTTPS를<br/>라우팅·종료하나?"}
  Q1 -->|내부| I1{"HTTP/HTTPS를<br/>라우팅하나?"}
  E1 -->|"예 (L7)"| EA["Global External App LB<br/>(글로벌 IP·CDN·WAF)<br/>또는 Regional External App LB<br/>(데이터 거주)"]
  E1 -->|"아니오 (L4)"| E2{"클라이언트 IP 보존·<br/>초저지연·비TCP?"}
  E2 -->|"예"| EP["External Passthrough NLB<br/>(리전 전용)"]
  E2 -->|"아니오 · 글로벌 IP/Armor"| EPX["External Proxy NLB<br/>(글로벌 가능)"]
  I1 -->|"예 (L7)"| IA["Internal Application LB"]
  I1 -->|"아니오 (L4)"| I2{"커스텀 라우트<br/>next hop / IP 보존?"}
  I2 -->|"예"| IP["Internal Passthrough NLB"]
  I2 -->|"아니오 · 프록시 분산"| IPX["Internal Proxy NLB"]
```
...
<div class="callout-warning">
결정트리의 함정: <strong>②에서 L4로 내려간 순간 Cloud CDN·Cloud Armor WAF는 후보에서 사라진다</strong>(패스스루 L4엔 WAF 미적용, CDN은 Application LB 전용). 그래서 "WAF/CDN이 필요하다"는 요구가 하나라도 있으면 ②는 자동으로 L7(Application)으로 확정된다. L4를 먼저 고른 뒤 "여기에 WAF를 붙이자"는 역순 설계가 대표 오답이다.
</div>
...
#### 엣지 기능 × LB 결합 매트릭스 — 함정 표

CDN과 Armor가 "아무 LB에나 붙는다"는 오해가 오답의 핵심이다. 어느 LB에 무엇이 붙는지를 한 장으로 못박는다.

| LB 유형 | Cloud CDN | Cloud Armor WAF(L7) | Cloud Armor 네트워크 DDoS |
|---------|-----------|---------------------|---------------------------|
| **Global External Application LB** | 붙음(백엔드 서비스/버킷 토글) | 붙음(backend/edge policy) | 해당 없음(L7 프록시) |
| **Regional External Application LB** | 안 붙음(CDN은 글로벌 App LB 전용) | 붙음(리전 backend policy) | 해당 없음 |
| **External Proxy Network LB** | 안 붙음 | 붙음(프록시라 L7 정책 가능) | 해당 없음 |
| **External Passthrough Network LB** | 안 붙음 | **안 붙음(L4)** | 네트워크 DDoS 방어(상위 등급)로 별도 |
| **Internal LB(App/Passthrough)** | 안 붙음(내부 전용) | 내부 App LB 한정 부분 지원 | 해당 없음 |

<div class="callout-warning">
가장 많이 틀리는 두 줄: <strong>① Cloud CDN은 Global External Application LB에만 붙는다</strong> — Regional App LB나 Proxy/Passthrough NLB에는 붙지 않는다. "특정 리전 고정 + CDN 캐싱"을 동시에 요구하면 모순이므로 지문을 다시 읽어야 한다(대개 CDN이 필요하면 글로벌 App LB로 수렴). <strong>② Passthrough L4에는 WAF가 없다</strong> — 여기에 필요한 건 WAF가 아니라 네트워크 DDoS 방어다.
</div>
...
## 공식 케이스 스터디 연결 — 이 도메인이 나오는 곳

PCA 시험은 4개 공식 케이스 스터디(EHR Healthcare / Helicopter Racing League / Mountkirk Games / TerramEarth)를 두고 케이스당 여러 문항을 낸다. LB·연결 도메인과 정면으로 맞닿는 것은 **글로벌 스트리밍의 HRL**과 **글로벌 저지연 게임의 Mountkirk**다. (케이스 세부 수치는 공식 v6.1 원문 기준으로, 아래는 도메인 매핑에 집중한 요약이다.)

### Helicopter Racing League (HRL) — 글로벌 스트리밍·엣지·CDN

핵심 요구는 "전 세계 시청자에게 낮은 지연으로 실시간·주문형 콘텐츠를 배포하고, 예측(AI)을 강화하며, 관측성을 높인다"이다. LB·엣지 관점의 매핑:

| HRL 요구사항 | 이 글의 프리미티브 | 왜 |
|--------------|--------------------|-----|
| 전 세계 시청자, 단일 진입점, HTTP/HTTPS 스트리밍 API | **Global External Application LB** | 글로벌 anycast 단일 IP + L7 |
| 콘텐츠 글로벌 배포·지연 최소화·origin 부하 감소 | **Cloud CDN**(대용량 미디어는 Media CDN 고려) | 정적·미디어 캐싱은 글로벌 App LB 위에서만 |
| 공개 엔드포인트 DDoS·악성 요청 방어 | **Cloud Armor**(글로벌 App LB 백엔드) | 엣지 WAF/DDoS는 프록시/App LB에만 결합 |
| 실시간 예측(edge inference)·낮은 지연 | 리전 분산 배포 + GPU(별도 도메인) | LB는 글로벌, 추론은 사용자 근처 리전 |

<div class="callout-tip">
HRL은 이 글의 정답 3종("글로벌 App LB + Cloud CDN + Cloud Armor")이 한 번에 모이는 대표 케이스다. "글로벌 스트리밍 + CDN + WAF"가 지문에 보이면 L4/패스스루·Regional App LB는 자동 탈락하고 <strong>Global External Application LB로 수렴</strong>한다.
</div>

### Mountkirk Games — 글로벌 저지연 멀티플레이(L4)와 HTTP 계층의 분리

핵심 요구는 "모바일 멀티플레이어 세션형 게임을 글로벌 동시 출시, 저지연 멀티플레이, GKE 기반 오토스케일, 실시간 분석"이다. 여기서 함정은 **게임 트래픽(L4)과 API·매치메이킹 트래픽(L7)을 같은 LB로 묶으려는 유혹**이다. 둘은 계층이 다르므로 LB도 갈린다.

| Mountkirk 요구사항 | 이 글의 프리미티브 | 왜 |
|--------------------|--------------------|-----|
| 실시간 멀티플레이 세션(저지연 UDP/TCP, 클라이언트 IP·지역성 중요) | **External Passthrough Network LB**(리전별, 멀티리전 배포) | 패스스루는 IP 보존·초저지연, 단 **리전 전용** |
| 비HTTP TCP에 **단일 글로벌 IP**가 필요한 경우 | **External Proxy Network LB**(글로벌) | 글로벌 단일 IP는 패스스루 불가 → 프록시 |
| 매치메이킹·리더보드·프로필 등 HTTP API | **Global External Application LB** | L7 라우팅·글로벌 IP |
| GKE 오토스케일 백엔드 | LB 백엔드(NEG) + GKE(별도 도메인) | 컴퓨트/오케스트레이션은 04·06 노트 |

<div class="callout-warning">
Mountkirk의 대표 함정: "<strong>글로벌 저지연 게임</strong>"만 보고 패스스루 NLB에 <strong>단일 글로벌 IP</strong>를 기대하는 것. 패스스루는 <strong>리전 전용</strong>이라 전역 단일 IP가 없다 — 멀티리전 배포 + DNS/지역 라우팅으로 근접 리전에 붙이거나, 비HTTP TCP에 굳이 하나의 글로벌 IP가 필요하면 <strong>Proxy Network LB(전역)</strong>로 간다. UDP 실시간 트래픽은 패스스루(리전)로, 그 위 HTTP API는 Global App LB로 계층을 나누는 것이 정답 패턴이다.
</div>

**결론**: HRL은 "글로벌 App LB + CDN + Armor" 3종 수렴형, Mountkirk은 "L4 게임 트래픽(패스스루/프록시) + L7 API(글로벌 App LB) 분리"형이다. 케이스에서 요구를 계층(L7/L4)·범위(글로벌/리전)로 먼저 쪼개면 LB가 자동으로 갈린다.
...
**Q7. 구명칭 매핑 — 지문에 "HTTP(S) Load Balancer"**

레거시 문서 기반의 문항이 "전 세계 사용자에게 **HTTP(S) Load Balancer**로 콘텐츠를 서비스하고 Cloud CDN을 활성화한다"고 서술한다. 이 구성이 현행 명칭으로 가리키는 것과, CDN 활성화가 가능한 이유로 가장 정확한 것은?

- (A) Regional External Application LB — 리전 고정이라 CDN 캐싱에 유리하다.
- (B) Global External Application LB — CDN은 이 글로벌 App LB의 백엔드에 토글로 붙는다.
- (C) External Proxy Network LB — TCP 프록시라 CDN이 붙는다.
- (D) External Passthrough Network LB — 클라이언트 IP 보존으로 캐싱이 정확해진다.
...
구명칭 "(External) HTTP(S) Load Balancing"은 현행 **Global External Application LB**다. Cloud CDN은 바로 이 글로벌 External Application LB의 백엔드 서비스/버킷에 토글로 활성화된다.

- (A) Regional App LB에는 Cloud CDN이 붙지 않는다(CDN은 글로벌 App LB 전용). "리전 고정이라 캐싱 유리"는 근거가 틀림
- (C) Proxy Network LB(구 TCP/SSL Proxy)는 L4로 HTTP를 해석하지 않으며 CDN 대상이 아님
- (D) Passthrough NLB(구 Network Load Balancing)에는 CDN·WAF가 붙지 않음
...
**Q8. Cloud CDN과 Regional App LB의 모순**

규제로 트래픽을 **단일 리전에서만** 처리해야 하는 HTTP 서비스가 있다. 동시에 팀은 "Cloud CDN으로 글로벌 캐싱"을 요구한다. 아키텍트로서 가장 정확한 판단은?

- (A) Regional External Application LB에 Cloud CDN을 켜면 두 요구를 모두 만족한다.
- (B) 두 요구는 상충한다 — Cloud CDN은 Global External Application LB에만 붙고, 데이터 거주(단일 리전 고정)는 Regional App LB의 특성이다. 요구 우선순위를 재확인해야 한다.
- (C) External Proxy Network LB로 바꾸면 리전 고정과 CDN을 동시에 얻는다.
- (D) Cloud CDN 대신 VPC Peering으로 캐싱을 구현한다.
...
Cloud CDN은 **Global External Application LB 전용** 기능이고, 데이터 거주(단일 리전 처리)는 **Regional External Application LB**로 푸는 요구다. 하나의 LB가 둘을 동시에 만족할 수 없으므로, 실제 시험에선 "규제상 리전 고정이 절대 조건"인지 "글로벌 캐싱이 우선"인지 요구 우선순위를 읽어 하나를 택해야 한다.

- (A) Regional App LB엔 CDN이 붙지 않음
- (C) Proxy NLB는 L4라 CDN·데이터 거주형 L7 라우팅과 무관
- (D) VPC Peering은 내부 VPC 연결이지 콘텐츠 캐싱 도구가 아님
...
**Q9. Mountkirk — 글로벌 게임 트래픽의 계층 분리**

Mountkirk Games가 실시간 멀티플레이(저지연 UDP 세션 트래픽)와 매치메이킹·리더보드 HTTP API를 글로벌로 서비스한다. 가장 적절한 LB 조합은?

- (A) 게임·API 모두 Global External Application LB 하나로 통합한다.
- (B) UDP 세션은 리전별 External Passthrough Network LB(멀티리전 배포), HTTP API는 Global External Application LB로 분리한다.
- (C) 게임·API 모두 단일 글로벌 IP의 External Passthrough Network LB로 처리한다.
- (D) 게임·API 모두 Internal Passthrough Network LB로 처리한다.
...
계층이 다르면 LB도 갈린다. UDP 실시간 세션은 **패스스루 NLB**(클라이언트 IP 보존·초저지연, 단 리전 전용 → 멀티리전 배포 + 지역 라우팅)로, HTTP API는 **Global External Application LB**(L7·글로벌 IP)로 처리한다.

- (A) Application LB는 HTTP/S 전용이라 UDP 게임 트래픽을 다루지 못함
- (C) Passthrough는 **리전 전용**이라 단일 글로벌 IP가 없음 — "단일 글로벌 IP passthrough"는 존재하지 않는 구성
- (D) Internal LB는 VPC 내부 전용 — 외부 글로벌 플레이어 부적합
...
**Q10. HRL — 글로벌 스트리밍 3종 수렴**

Helicopter Racing League가 전 세계 시청자에게 HTTP 스트리밍을 낮은 지연으로 배포하고, origin 부하를 줄이며, 공개 엔드포인트를 DDoS·SQLi로부터 보호하려 한다. 한 번에 맞는 구성은?

- (A) External Passthrough Network LB + Cloud IDS + VPC 방화벽
- (B) Global External Application LB + Cloud CDN + Cloud Armor
- (C) Regional External Application LB + Cloud CDN + Cloud Armor
- (D) External Proxy Network LB + Cloud CDN + Cloud Armor
...
"글로벌 HTTP + 지연·origin 부하 감소(CDN) + DDoS/WAF(Armor)"는 이 글의 3종 수렴형이다. 세 기능 모두 **Global External Application LB** 위에서만 함께 성립한다.

- (A) 패스스루 L4엔 CDN·WAF가 없고, IDS는 탐지 전용이라 DDoS 차단이 아님
- (C) Regional App LB엔 Cloud CDN이 붙지 않음(글로벌 App LB 전용)
- (D) Proxy Network LB는 L4라 Cloud CDN이 붙지 않음
...
Mountkirk의 분석 파이프라인 일부가 **AWS에 남아** 있고, 이 데이터를 GCP로 **공용 인터넷을 거치지 않고** 사설·고대역으로 지속 전송해야 한다. 올바른 연결 방식은?

- (A) Dedicated Interconnect — 온프렘·클라우드 구분 없이 모든 사설 연결에 쓴다.
- (B) Cross-Cloud Interconnect — 다른 퍼블릭 클라우드(AWS 등)와 GCP를 전용 물리 회선으로 사설 연결한다.
- (C) HA VPN — 인터넷 위 IPsec으로 AWS와 잇는다.
- (D) VPC Peering — AWS VPC와 GCP VPC를 직접 피어링한다.
...
"다른 퍼블릭 클라우드(AWS) ↔ GCP를 사설로"는 **Cross-Cloud Interconnect**다. Dedicated/Partner Interconnect는 **온프렘 ↔ GCP**용이다.

- (A) Dedicated Interconnect는 온프렘 전용 — 타 클라우드 대상이 아님
- (C) HA VPN은 **인터넷 경유** IPsec이라 "공용 인터넷을 거치지 않는다"는 요구에 어긋남
- (D) VPC Peering은 **GCP VPC 사이** 연결이지 타 클라우드와 잇는 도구가 아님
...
**Q12. 연결 방식 선택 — 빠른 구축·암호화·중대역**

온프렘과 GCP를 잇는데 ① 몇 주 안에 빠르게 시작 ② 전송 구간 암호화 필수 ③ 대역폭은 수 Gbps면 충분하다. Google POP까지는 물리 회선을 새로 끌기 어렵다. 가장 적합한 선택은?

- (A) Dedicated Interconnect — 최고 대역폭·저지연이므로 항상 우선한다.
- (B) HA VPN — 인터넷 위 IPsec으로 빠르게 구축하고 암호화가 기본 제공된다.
- (C) Partner Interconnect — 기본 암호화가 제공되어 규제를 만족한다.
- (D) Cross-Cloud Interconnect — 온프렘 연결의 표준이다.
...
빠른 구축 + 전송 암호화 기본 + 중간 대역이면 **HA VPN**이 정석이다. IPsec으로 암호화가 기본 제공되고 물리 회선 프로비저닝이 필요 없어 도달 시간이 짧다.

- (A) Dedicated Interconnect는 물리 회선 프로비저닝으로 구축이 느리고 기본 비암호화(MACsec은 별도)이며, POP까지 회선 확보가 어렵다는 전제에도 어긋남
- (C) Partner Interconnect는 **기본 비암호화**(사설 회선 전제) — "기본 암호화"가 오답 포인트
- (D) Cross-Cloud Interconnect는 **타 클라우드↔GCP**용이지 온프렘 연결 도구가 아님
