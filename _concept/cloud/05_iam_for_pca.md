...
이 글의 지도: 기본 모델(Principal·Role·Policy) → 역할 3종 → 리소스 계층과 상속 → Service Account → Workload Identity Federation → 직무 분리(SoD) → Org Policy vs IAM → IAM Conditions → 시험 공략 → 퀴즈. 각 축은 "함정 → 판단 기준 → 결론"으로 닫는다.
</div>
...
- Google Cloud, [*Workload Identity Federation*](https://cloud.google.com/iam/docs/workload-identity-federation) — 외부 IdP(AWS/Azure/OIDC) 키리스 연합
