...
00편이 레이어의 정의, 01편이 사고 분석이라면, 이 글은 변환 위치를 정하는 설계 결정 기준서다.
...
정리하면, 00편은 레이어의 정의, 01편은 소스 변경을 흡수하지 못한 사고 분석, 이 글은 변환을 어느 레이어에 둘지 판단하는 룰북이다. 위 여섯 질문이 그 판단의 핵심이다.

이어서 다룰 주제는 거버넌스와 데이터 리니지 — 레이어를 넘나드는 데이터의 출처와 흐름을 추적하는 일이다.
