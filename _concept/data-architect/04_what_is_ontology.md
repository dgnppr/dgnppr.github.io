...
그 결과 의미는 코드 곳곳과 분석가의 머릿속(tribal knowledge)에 흩어진다. 분석가 A는 활성 고객을 90일로, B는 180일로 쓴다. 시간이 지나면 정의는 조용히 갈라진다. 나는 이것을 **의미의 드리프트(semantic drift)** 라고 부른다. 지표가 틀린 게 아니라, 같은 이름의 지표가 서로 다른 세계를 가리키게 되는 것. 데이터 거버넌스가 회의 때마다 무너지는 진짜 이유는 여기에 있다.
