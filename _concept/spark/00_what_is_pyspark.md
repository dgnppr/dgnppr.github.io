...
작업은 다음과 같다. GCS에 Parquet로 저장된 8억 행, 압축 후 약 400GB의 주문 로그에서 고객별·일별 매출을 집계한다. 가용 메모리는 16GB다.
...
Spark와 PySpark는 서로 다른 계층이다. Spark는 Scala로 작성돼 JVM 위에서 실행되는 분산 엔진이고, PySpark는 그 엔진을 호출하는 파이썬 API다. 둘은 같은 프로세스가 아니다.
...
Spark 클러스터는 세 역할로 구성된다.

- **Driver** — 사용자 코드가 실행되는 프로세스. 연산 계획을 세우고, 분할된 일감을 익스큐터에 분배하며, 익스큐터의 진행 상황을 추적한다.
- **Executor** — 데이터 조각을 들고 실제 계산을 수행하는 워커 프로세스. 여러 노드에 분산돼 병렬로 실행되고, 캐시된 데이터를 자신의 메모리에 보관한다.
- **Cluster Manager** — Driver의 요청을 받아 노드에 익스큐터를 할당하는 자원 관리자. YARN·Kubernetes·Spark Standalone 중 하나다. Spark의 실행 로직과 분리돼 있어, 같은 애플리케이션 코드가 어느 매니저 위에서도 동일하게 동작한다.
...
Spark 3.x의 AQE는 셔플 후 지나치게 작아진 파티션들을 실행 시점에 자동 병합(coalesce)한다. 따라서 이 값을 크게 잡아 생기는 파편화는 어느 정도 자동 보정된다. 반면 값이 너무 작아 파티션 하나가 과대해지는 문제는 AQE가 되돌리지 못하므로, 이 값은 데이터 규모에 넉넉히 두고 세부 병합은 AQE에 맡기는 편이 안전하다.
