...
> 두 시리즈가 같은 회사 마켓온을 다뤘다. [[/data-architect/00_what_is_medaliion_architecture]]부터 [[/data-architect/03_medallion_advanced_patterns]]까지는 데이터가 Bronze·Silver·Gold를 거쳐 정제되는 과정을, [[/data-architect/04_what_is_ontology]]부터 [[/data-architect/07_ontology_core_concepts]]까지는 그 데이터가 의미를 갖는 과정을 다뤘다. 이 글은 두 시리즈가 사실 한 스택의 아래층과 위층임을 보인다.
...
1차 장애는 메달리언이 막을 수 있었다. 2차 장애는 메달리언만으로는 막을 수 없었다. **신뢰 가능한 행이 신뢰 가능한 세계는 아니기 때문이다.** 그 간극이 이 글의 출발점이다.
...
여기서 메달리언과 온톨로지의 결합이 마지막 의미를 갖는다. 에이전트가 신뢰할 수 있는 행동을 하려면, 그 아래 모든 층이 신뢰 가능해야 한다 — Bronze가 원본을 보존하고(추적성), Silver가 표준화하고(1차 장애 방지), 온톨로지가 entity·link·action을 모델링하고(2차 장애 방지), 그제서야 에이전트는 K씨를 *완전하게* 본다. **에이전트의 월드모델은 메달리언 스택 전체의 신뢰도 위에 선다.**
