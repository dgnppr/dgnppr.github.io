...
메달리언은 개별 행(行)을 신뢰 가능하게 만들고, 온톨로지는 그 행이 가리키는 대상 — 엔티티·관계·행위 — 을 신뢰 가능하게 만든다. 둘은 경쟁하지 않는다. **온톨로지는 메달리언을 대체하지 않고, 메달리언의 Silver 위에 쌓인다.** 이 글은 그 적층 지점이 어디이고 왜 거기인지, 그리고 메달리언의 패턴이 온톨로지 구조로 어떻게 재사용되는지를 설명한다.

---

## 같은 회사, 두 장애, 두 레이어

마켓온에서 발생한 두 건의 데이터 장애는 성격이 달랐고, 해결책이 위치한 레이어도 달랐다. 둘을 나란히 두면 메달리언과 온톨로지의 경계가 드러난다.

**첫 번째 장애** — [[/data-architect/01_how_to_architect_medallion_well]]. ERP가 주문 상태 코드를 `'C'`에서 `'CM'`으로 바꾸자 47개 쿼리가 동시에 깨지며 GMV가 0이 됐다. 원인은 코드 변경이 아니라 그 변경을 흡수할 레이어의 부재였다. 답은 Silver였다 — 소스의 언어(`'C'`, `'CM'`)를 비즈니스의 언어(`is_completed`)로 번역하는 한 곳.

**두 번째 장애** — [[/data-architect/06_how_to_implement_ontology]]. 이탈 모델이 VIP K씨를 위험군으로 분류해 쿠폰을 발송했지만, K씨는 이틀 전 오프라인 매장에서 51만 원을 결제한 충성 고객이었다. CRM의 `U-29182`와 POS의 `M-00991`이 같은 사람이라는 걸 어떤 시스템도 알지 못했다. 답은 온톨로지였다 — `canonical_id`, `placed` 링크, 그리고 `action_def` 게이트.

두 장애를 나란히 두면 경계가 드러난다.

| | 1차 장애 (ERP 'CM') | 2차 장애 (K씨 채널전환) |
|--|---------------------|------------------------|
...
1차 장애는 메달리언이 막을 수 있었다. 2차 장애는 메달리언만으로는 막을 수 없었다. **신뢰 가능한 행이 곧 신뢰 가능한 엔티티는 아니기 때문이다.** 이 간극이 결합 아키텍처가 채우는 자리다.
...
[[/data-architect/04_what_is_ontology]]의 용어로 말하면, Silver에는 **명사는 흩어져 있고, 동사와 시간은 없다.** 명사·동사·시간을 모델링하는 일이 메달리언의 책임 밖이고, 온톨로지의 책임이다.
...
1. **온톨로지는 Silver를 입력으로 삼는다.** Object의 Primary Key(`canonical_id`)는 Silver의 entity resolution 결과(`canonical_customer_id`)다. Property 값(`is_completed`)은 Silver의 표준화 결과다. 온톨로지는 Bronze의 원본 코드(`'CM'`)를 절대 직접 보지 않는다 — 그건 Silver가 흡수한다.
...
동일성 하나만 겹치는 게 아니다. [[/data-architect/03_medallion_advanced_patterns]]가 SQL로 굳힌 패턴들이 거의 그대로 온톨로지 구조로 옮겨진다. 같은 메커니즘을 다른 대상에 적용하는 것이다.
...
## Gold 재해석 — metric은 온톨로지 위에서 집계된다
...
이 쿼리는 1차 장애를 막는다('CM'을 Silver가 흡수했으므로). 하지만 2차 장애는 못 막는다 — `source_system`별로 GMV를 나눠도, K씨의 CRM GMV와 POS GMV가 *같은 사람의 것*이라는 사실은 여기 없다. Gold가 Silver 위에서 집계하는 한, 집계의 단위는 여전히 *행*이지 *엔티티*가 아니다.
...
결합 스택의 최상위 소비자는 AI 에이전트다. [[/data-architect/04_what_is_ontology]] §7과 06편이 보였듯, 에이전트는 Bronze의 테이블 1,000개 스키마를 보지 않는다. 온톨로지가 주는 객체·관계·허용된 행위만 본다.
...
결합의 효과는 이 지점에서 분명해진다. 에이전트가 신뢰할 수 있는 행동을 하려면, 그 아래 모든 층이 신뢰 가능해야 한다 — Bronze가 원본을 보존하고(추적성), Silver가 표준화하고(1차 장애 방지), 온톨로지가 entity·link·action을 모델링하고(2차 장애 방지), 그제서야 에이전트는 K씨를 *완전하게* 본다. **에이전트의 월드모델은 메달리언 스택 전체의 신뢰도 위에 선다.**
...
표준화를 건너뛰고 raw 위에 Object를 세우면, 소스의 언어가 온톨로지로 새어든다. ERP의 `'CM'`이 그대로 Object Property가 되고, 1차 장애가 온톨로지 층에서 재발한다.
...
-- ✓ Silver를 입력으로: 표준화된 값만 Object property로
...
[[/data-architect/04_what_is_ontology]] §8의 최상위 실패 모드(프로젝트 취급)가 결합 맥락에서 더 날카로워진다. 온톨로지를 Silver와 동기화하지 않으면, Silver가 재처리될 때 온톨로지가 *옛 상태*를 가리킨다. 메달리언의 재처리가 오히려 온톨로지를 깨뜨린다.
...
마켓온의 두 장애는 한 회사의 서로 다른 레이어에서 발생했다. ERP `'CM'`은 Silver가 막았어야 했고, K씨 채널전환은 온톨로지가 막았어야 했다. 둘을 따로 읽으면 별개의 교훈이지만, 한 스택으로 겹쳐 읽으면 하나의 원칙으로 모인다.

> 메달리언은 데이터가 깨끗한지를 묻고, 온톨로지는 그 데이터가 무엇을 뜻하는지를 묻는다. 신뢰는 아래에서 위로 쌓인다 — 원본의 보존에서, 행의 표준화로, 엔티티의 모델링으로, 행위의 거버넌스로.

온톨로지는 메달리언을 대체하지 않는다. Silver가 끝나는 곳에서 시작하고, 그 위에 metric이, 다시 그 위에 에이전트가 놓인다. 한 층이라도 틀리면 그 위 모든 층이 틀린다. 그래서 결합 아키텍처의 규칙은 단순하다 — **각 층은 바로 아래 층만 신뢰하고, 자기 책임만큼만 변환한다.**
...
- [[/data-architect/01_how_to_architect_medallion_well]] — ERP 'C'→'CM' 장애와 Silver의 번역 책임, `marketeon.ontology.customer_identity`의 첫 등장
