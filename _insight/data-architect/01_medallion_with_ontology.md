...
메달리언은 개별 행을 신뢰 가능하게 만들고, 온톨로지는 그 행이 가리키는 대상 — 엔티티·관계·행위 — 을 신뢰 가능하게 만든다. 둘은 경쟁하지 않는다. **온톨로지는 메달리언을 대체하지 않고, 메달리언의 Silver 위에 쌓인다.** 이 글은 그 적층 지점이 어디이고 왜 거기인지, 그리고 메달리언의 패턴이 온톨로지 구조로 어떻게 재사용되는지를 설명한다.
