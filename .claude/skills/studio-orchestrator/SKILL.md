...
입력 "Iceberg의 hidden partitioning이 왜 중요한가" → architect가 `concept/data-architect`에 배치, `research_required:false`, 기존 medallion 노드에 `extends` 연결 설계 → sme가 파티셔닝 함정·코드 작성 → writer가 직설형 기술 본문 → editor가 `_concept/data-architect/NNNN_iceberg_hidden_partitioning.md` 발행 + relations 검증 + `make ontology` + 다음 글로 "Iceberg vs Hudi" 제안.
