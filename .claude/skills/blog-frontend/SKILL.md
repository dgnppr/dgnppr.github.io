---
name: blog-frontend
description: "테크 블로그 Jekyll 구현 스킬. tech-blogger 초안을 실제 포스트 파일로 구현(front matter·배치·빌드)하고, Jekyll 템플릿/Sass 수정, JS 기능 구현을 수행한다. '포스트 구현', '초안 반영', '구현해줘', '템플릿 수정', 'CSS 추가', 'front matter', 'generateData', Jekyll 관련 작업 시 반드시 이 스킬을 사용. 스타일 보완, 기능 추가 요청에도 사용. (본문 산문 작성은 tech-blogger 스킬이 담당)"
---
...
본문 산문은 tech-blogger가 작성한다. Frontend는 초안을 실제 Jekyll 포스트로 구현한다.

### 1. 입력 파일 읽기

- `_workspace/03_blogger_draft.md` — **본문 초안 (구현 대상)**
- `_workspace/02_designer_spec.md` — 마크다운 템플릿 및 포맷 가이드
- `_workspace/01_tpo_brief.md` — 배치 경로, 추천 태그, 대상 카테고리
...
초안의 본문은 그대로 살리고, Jekyll 구현 요소만 추가·정리한다 (본문 산문은 임의로 다시 쓰지 않는다):
- TOC는 위키 포스트에 `* TOC\n{:toc}` 형식으로 추가 (designer 스펙의 TOC 위치 표시 적용)
- 코드블록 언어 식별자 누락 시 보완: ` ```python `, ` ```sql `, ` ```yaml `, ` ```bash ` 등
- front matter 필드 정확히 작성 (아래 2번 참조)
- 빌드/포맷 충돌이 있으면 tech-blogger에게 확인 후 수정
