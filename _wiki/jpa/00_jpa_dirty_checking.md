...
`EntityManager`는 `Entity`의 영속성을 관리하는 인터페이스이다. `EntityManager`는 내부에서 `PersistenceContext`를 가지고 있으며, 이 객체를 통해 `Entity`의 영속성을 관리한다.
...
`SessionImpl`은 `EntityManager`의 구현체이다. 위 코드를 보면 내부에 `private transient StatefulPersistenceContext persistenceContext;`가 존재하는 것을 확인할 수 있다.
...
다른 트랜잭션에서 동작하기 때문에 서로 다른 `txObject`를 가지고, 내부에서 서로 다른 `EntityManager`를 사용한다. **당연한 소리겠지만 다른 트랜잭션에서는 서로 다른 `PersistenceContext`를 가지고 있다.**
...
2. `DefaultFlushEventListener`에서는 `this.flushEntities(event, persistenceContext)`를 호출하여,  `DefaultFlushEntityEventLister`를 호출한다.
3. `DefaultFlushEntityEventLister`에서는 `persister.dirtyCheck()`를 호출하여 `Entity`의 변화를 감지한다.
...
### DefaultFlushEntityEventLister

`DirtyCheck`를 하기 위해서는 snapshot을 가지고 있어야 한다. `DefaultFlushEntityEventLister`에서는 `persister`를 통해 `Entity`의 snapshot을 가져옴을 아래 코드에서 확인할 수 있다.
...
- 코드 명확성 저하
  ...
  - `repository.save()`를 직접 명시하지 않고 커스텀하게 엔티티의 속성 값을 변경시키기 때문에 엔티티의 변화 추적이 어렵다.
- 테스트 어려움
  - 그냥 엔티티 객체가 아니라, 영속성 컨텍스트에 적재된 엔티티 객체가 변화되었음을 테스트해야 하므로, 테스트가 어려워진다.
