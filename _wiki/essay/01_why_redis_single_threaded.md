...
아래와 같은 대답에서 어떤 관점에서 싱글 스레드로 개발했는지 알 수 있었다.
...
레디스가 하나의 메인 스레드만 있을까? 그것은 아니다.

```Shell
ps -ef | grep "redis"
```

레디스가 동작하는 환경에서 프로세스를 조회해보면 하나의 메인 스레드만 동작하지 않는 것을 확인할 수 있다.
...
> "Non blocking DEL and FLUSHALL/FLUSHDB" There is a new command called UNLINK that just deletes a key reference in the database, and does the actual clean up of the allocations in a separated thread
...
나는 레디스가 멀티 스레드로 동작한다면 지금보다도 훨씬 높은 성능을 보일 것 같다. 물론 동기화, 메모리 이슈(`fork`로 인한)가 발생하겠지만, 메모리에 저장되는 자료구조(ex: 해시테이블 등)는 비교적 간단하고, 메모리 접근이 매우 빠르고, 낮은 경합이 이뤄질 것이라고 보기 때문에
동기화를 처리하는 디메리트보다 성능 개선이 더 메리트를 가질 것이라고 생각한다. 

멀티 쓰레드를 사용하게 되면 메모리 사용량이 증가할텐데, 이 부분에서 레디스에서 고민이 되지 않을까 싶다. 메모리를 최대한 활용하기 위해서 쓰레드 개수를 최소화하고, 메모리를 효율적으로 사용하는 자료 구조를 사용하면 좋을 것 같다.
유저 모드에서도 쓰레드풀을 도입하여 적은 수의 쓰레드로 최대한 활용하고, 링 버퍼와 같은 자료구조를 사용하여 고정 크기의 버퍼에 데이터를 저장하여 메모리 단편화을 줄이고, 스택과 힙의 크기를 동적으로 할당하여, 메모리를 관리하면 좋을 것 같다.
