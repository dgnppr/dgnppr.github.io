...
> [[/data-architect/01_how_to_implement_ontology]] 에서 "Customer는 *엔티티*다", "구매를 *관계*로 표현한다", "쿠폰 발송은 *행위*다"라고 썼다. 그 단어들을 엄밀히 구분하지 않은 채로. 1편이 알람 하나로 온톨로지를 훑었다면, 이 글은 그 안에서 매번 미끄러졌던 개념들의 경계를 못 박는다. 그것도 *추상*이 아니라 — 당신이 지금 브라우저에서 직접 실행할 수 있는 실존 시스템 위에서.

---

## 도입 — 같은 개념, 정반대 진영

1편은 마켓온의 K씨 사건을 끝까지 파고들었다. canonical ID로 K씨를 하나로 묶고, `placed` 링크로 전채널 구매를 통합하고, `prop_def`에 정의의 이력을 남기고, `action_def`로 쿠폰에 게이트를 달았다. 동작하는 온톨로지였다. 그런데 그 글은 *엔티티*, *객체*, *타입*, *인스턴스*, *관계*를 거의 동의어처럼 흘려 썼다.

마켓온은 **폐쇄 세계**였다. 조직 경계 안의 규범 데이터, 커스텀 SQL, CWA(닫힌 세계 가정). RDF도 OWL도 SPARQL도 쓰지 않았다. 1편은 "왜 표준 스택을 *안 쓰고* SQL을 쓰는가"를 논증한 글이었다.

이 글은 정반대편으로 간다. **Wikidata** — 위키미디어 재단의 협업형 지식베이스이자, 세계 최대의 *개방형* 온톨로지다. 1억 개가 넘는 item, 누구나 편집, 전체가 RDF로 노출되고 SPARQL로 질의된다. 마켓온이 폐쇄·CWA·커스텀이라면, Wikidata는 개방·OWA·표준이다.

<div class="callout-note">
이 글의 전략: 마켓온(1·2편)과 Wikidata는 <strong>대조쌍</strong>이다. 같은 다섯 개념(명사·관계·속성·표준·시제)이 양 극단에서 어떻게 형식화되는지 겹쳐 보면, 각 개념의 경계가 평면이 아니라 입체로 드러난다. 0편의 명제 — "도구가 아니라 문제가 정당화한다" — 를 양쪽에서 입증하는 것이 목표다.
</div>

Wikidata의 모든 것은 두 가지로 표현된다. **item**(Q-number, 예: `Q42` = Douglas Adams)과 **property**(P-number, 예: `P31` = instance of). 시맨틱 웹 커뮤니티의 "Hello World"는 이 한 줄이다.

> **Q42 P31 Q5** — "Douglas Adams는 human의 instance다."

이 한 triple 안에 2편이 정의할 네 개의 축이 전부 들어 있다. `Q42`(명사), `P31`(관계), 이 줄이 곧 RDF triple이라는 사실(표준), 그리고 — 뒤에서 보겠지만 — 이 사실이 *언제 참인가*(시제). 네 축을 차례로, 각각 query.wikidata.org에서 실행 가능한 쿼리와 함께 판다.

<div class="callout-info">
이 글의 모든 SPARQL은 <code>https://query.wikidata.org</code> 에 그대로 붙여넣어 실행할 수 있다. prefix(<code>wd:</code> item, <code>wdt:</code> truthy property, <code>p:</code>/<code>ps:</code>/<code>pq:</code> reified statement)는 WDQS에 기본 등록돼 있어 선언 없이 쓰면 된다. 결과 건수·구체 값은 Wikidata가 살아 있는 DB라 시점에 따라 달라질 수 있다 — 본문 수치는 "이런 형태로 나온다"는 예시다.
</div>

세 가지 질문에 답한다. Entity와 Object는 같은 말인가? Link는 그냥 외래키(FK)인가? 왜 마켓온은 RDF/OWL을 버렸는데 Wikidata는 그것으로 굴러가는가?

---

## 축 1 — 명사: Q-number로 보는 Entity·Object·Type·Instance

1편은 "Customer는 소스 시스템의 ID가 아니라 실체(사람)다"라고 했다. 이 한 문장 안에 네 개의 서로 다른 층위가 겹쳐 있다. 실세계의 사람, 그 사람을 표상하는 데이터베이스의 행(row), 그것이 속한 분류, 그리고 분류 안의 개별값. 2편이 표로 정리한 그 네 층위를, Wikidata는 *실물*로 보여준다.
...
<div class="callout-info">
2편 비교표의 "식별자" 행을 기억하라 — <code>IRI(global)</code> vs <code>canonical_id(org-local)</code>. <code>Q42</code>의 IRI <code>http://www.wikidata.org/entity/Q42</code> 가 바로 그 global 식별자다. 마켓온의 <code>CUST-029182</code>는 마켓온 조직 안에서만 유효하지만, <code>Q42</code>는 전 세계가 참조한다. 같은 "식별자"라는 단어가 두 개의 다른 스코프를 가리킨다.
</div>
...
그러면 "human이라는 Type의 외연(extension)"은 무엇인가? 2편은 "`customers` 테이블 자체가 'customer' Type의 외연"이라 했다. Wikidata에서 같은 것을 *쿼리로* 뽑을 수 있다.
...
2편은 "Ontology = 스키마(TBox)와 인스턴스(ABox)의 구분은 통상적인 것이지 정의상 고정된 것은 아니다"라고 했다. Wikidata가 이 논점을 살아 있는 형태로 보여준다. **하나의 item이 instance이면서 동시에 class일 수 있다.**
...
Honda Accord는 "car model"의 instance이고, "car model"은 그 자체로 class다. Wikidata는 class/instance를 *강제하지 않기* 때문에 한 item이 양쪽 역할을 가질 수 있다. 이것이 2편 TBox/ABox 논점의 실증이다 — 경계는 *관습*이지 *물리적 제약*이 아니다.
...
명사를 갈랐다. `Q42`는 Object, `Q5`는 Type, `Q42 P31 Q5`는 Type 선언, IRI는 전역 식별자. 다음은 이 명사들을 잇는 선 — 관계, 그리고 2편이 일부러 미뤄둔 *추론*이다.

---

## 축 2 — 관계: P31 vs P279, 그리고 2편이 미룬 추론을 실행한다

1편의 `links` 테이블을 처음 본 사람은 "이거 그냥 조인 테이블 아닌가?"라고 묻는다. 2편은 답했다 — 외래키는 *어느 행이 어느 행을 참조하는가*만 말하고, 관계의 **의미**와 **방향**은 말하지 못한다고. 그리고 2편은 결정적인 한 문장을 흘리고 지나갔다.

> "1편이 `is-a`나 `part-of`를 쓰지 않은 것은 우연이 아니다. 그 두 관계는 *추론(inference)*을 부르기 때문이다."

2편은 그 추론을 "뒤에서 다시 본다"고만 하고 넘어갔다. 여기서 정면으로 다룬다. Wikidata는 `is-a`를 *실제로* 쓰고, 그래서 그 추론을 *실행해* 보여줄 수 있다.

### P31 vs P279 — Instance/Type 경계가 두 property로 갈린다

2편이 가른 Instance/Type의 경계가, Wikidata에서는 두 개의 property로 분리돼 있다.
...
여기서 2편 "관계의 문법" 표의 그 `is-a`($\sqsubseteq$, subsumption)가 바로 **P279**다. 2편은 "이것을 적는 순간 reasoner는 추론한다"고만 했다. Wikidata 공식 문서는 그 추론 규칙을 명시한다.
...
이 "암묵적"이 2편이 말한 reasoner의 subsumption inference다. 그리고 SPARQL의 property path `*`(transitive closure)가 그것을 *명시적으로 실행*한다.
...
관계를 갈랐고, 미뤄둔 추론을 실행했다. 이제 이 모든 게 *어떤 스택* 위에서 도는지 — triple, reification, SPARQL의 실물로 간다.

---

## 축 3 — 표준 스택: SPARQL을 직접 실행한다

2편의 비교표를 다시 꺼낸다. 그 표의 *왼쪽 열*(RDF/OWL 스택)을 이번에는 *설명*이 아니라 *작동*시킨다.
...
2편은 "RDF는 세계를 triple(S-P-O)의 집합으로 본다"고 했다. 그 triple이 이것이다.
...
2편은 reification을 callout 한 줄로만 스쳤다("Foundry Object를 RDF reification과 혼동하지 말라"). 여기서 실물을 만진다.

문제는 이렇다. `Q42 wdt:P69 wd:Q691283`(Douglas Adams가 St John's College에서 *교육받았다*)는 단순 truthy triple이다. 그런데 "*언제* 다녔는가"라는 **qualifier**를 붙이려면 truthy(`wdt:`)로는 안 된다. statement를 하나의 *노드*로 끌어올려야 한다 — 이것이 reification이다.
...
스택을 작동시켰다. 마지막 축 — 시제. 여기서 OWA·rank·시간 qualifier가, 2편이 CWA로 설명한 시제를 *반대편에서* 보강한다.

---

## 축 4 — 시제: rank·P585로 시간을 형식화하고, OWA를 실제로 만지다

2편은 시제를 *마켓온의 CWA*로 설명했다 — `NOT EXISTS`(없으면 거짓), `valid_to IS NULL`(현재 유효), as-of 동결 vs 소급 재계산. Wikidata는 그 모든 논점을 *반대 진영*에서 실측하게 해준다.

### OWA를 직접 경험한다 — "없다"가 "거짓"이 아니다

2편의 표 한 칸: "기록에 없으면 — CWA=거짓, OWA=모름." 이것을 Wikidata에서 *손으로 만진다*.
...
여기에 2편 "valid_to ≠ CWA" 논점과 정확히 호응하는 핵심 통찰이 있다.
...
2편의 마지막 논점 — "정의 자체가 시간에 따라 바뀐다(의미 마이그레이션)" — 도 Wikidata에 있다. 단지 한 item의 값이 아니라, **class 구조 자체**가 드리프트한다. `P31`/`P279`로 짜인 분류 체계가 커뮤니티 합의에 따라 재편되면, 어제 `Q-something`의 instance였던 것이 오늘은 다른 class의 subclass가 된다. 2편 마켓온이 `prop_def`의 `valid_from`/`valid_to`로 정의 이력을 남긴 것의 *개방형 버전*이다 — 다만 Wikidata에서는 그 이력이 item 편집 히스토리에 흩어져 있어 마켓온의 `prop_def`처럼 *한 테이블에 정규화*돼 있지는 않다.
...
시제까지 갈랐다. OWA로 부재를 다루고, rank로 현재·과거·오류를 가르고, reified 경로로 시계열을 펼쳤다. 모두 마켓온 CWA의 *거울상*이다.
...
세 편을 지나왔다. 0편은 *why* — 온톨로지가 데이터가 아니라 합의를 설계한다는 주장. 1편은 *how* — 알람 하나의 인시던트를 다섯 개념의 구현으로 풀었다. 2편은 *what* — 그 개념들에 정확한 경계를 찍되, 이번엔 마켓온과 Wikidata라는 *두 극단*을 겹쳐 입체로 만들었다. 이제 당신은 온톨로지를 *말할* 수 있다. 그리고 `query.wikidata.org`를 열어 *직접 실행하며* 확인할 수 있다.
...
- [[/data-architect/00_what_is_ontology]] — 온톨로지가 합의를 설계한다는 주장, 시맨틱 웹 실패의 세 이유(OWA·형식화 비용·강제력 부재)
- [[/data-architect/01_how_to_implement_ontology]] — 마켓온 K씨 사건, 다섯 개념(Entity+Identity·Link·Property Definition·Action·Dynamic)의 폐쇄형 구현
