...
show-diagram: true
relations:
  - { type: extends, target: /wiki/data-architect/02_how_to_architect_medallion_well }
...
네 편을 지나왔다. 00편은 레이어가 무엇인지 그린 지도였고, 01편은 그 구조가 없어 GMV가 0이 된 사고 기록이었으며, 02편은 매일 경계를 판단하는 룰북이었다. 03편은 그 룰을 실행 가능한 SQL로 굳혔다 — 다섯 패턴은 룰의 실행체이고, 다섯 안티패턴은 룰이 무너지는 구체적 형태다.

패턴과 안티패턴까지 손에 익히면 한 가지가 남는다. 데이터가 Bronze에서 Gold까지 어떤 변환을 거쳐 어디로 흘러갔는지, 그 출처와 경로를 추적하는 일이다. 다음 편은 **데이터 리니지와 거버넌스** — 레이어를 넘나드는 데이터의 흐름을 기록하고 책임을 묶는 주제다(02편 L320 예고와 이어진다).
